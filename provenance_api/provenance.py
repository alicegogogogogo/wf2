...
build: who built it (``builder``), the build number (``build_id``), a
64-character hexadecimal source digest, and an ordered (possibly empty)
array of build materials, each with a name and digest. Digests are
accepted in mixed case but always stored and rendered in lowercase, and
materials preserve their submission order. Nothing here is persisted:
stopping or restarting the service clears every record, and no files are
written. Signatures are not verified and promotion rules are unchanged.
"""
...
#: Builder and build number are non-empty text bounded by this many Unicode
#: code points; material names share the same bound.
MAX_TEXT_LENGTH = 256

#: Fields accepted by the registration request; anything else is rejected.
#: All four fields are required (``materials`` may be an empty list).
_ALLOWED_FIELDS = frozenset(
    {"builder", "build_id", "source_digest", "materials"}
)

#: The only fields a material member may carry; both are required.
...
class ProvenanceMaterial:
    """A single build material identified by name and lowercase digest."""
...
class ProvenanceRecord:
    """The single build provenance record recorded against a resource."""
...
def _normalized_digest(value: object, label: str) -> str:
    """Validate a 64-character hexadecimal digest and lowercase it."""
...
def build_provenance_fields(
    payload: object,
) -> tuple[str, str, str, tuple[ProvenanceMaterial, ...]]:
    """Validate a decoded provenance payload and return normalized values.

    Returns ``(builder, build_id, source_digest, materials)``. The builder
    and build number are non-empty strings of at most 256 Unicode code
    points; the source digest is a 64-character hexadecimal string
    normalized to lowercase; ``materials`` is a tuple (possibly empty) of
    :class:`ProvenanceMaterial` members in their submitted order, each
    digest normalized to lowercase.

    A material member must be an object carrying exactly ``name`` and
    ``digest``; a repeated (name, digest) pair is an invalid request.
    """
    ...
    build_id = _required_text(payload["build_id"], "Field 'build_id'")
    source_digest = _normalized_digest(
    ...
    for position, raw_material in enumerate(raw_materials):
        ...
        if "name" not in raw_material:
            raise ProvenanceValidationError(
                f"Material at index {position} is missing required field: "
                "'name'."
            )
        if "digest" not in raw_material:
            raise ProvenanceValidationError(
                f"Material at index {position} is missing required field: "
                "'digest'."
            )
        ...
        if pair in seen:
            raise ProvenanceValidationError(
                f"Material at index {position} duplicates an earlier "
                "material (same name and digest)."
            )
        seen.add(pair)
        materials.append(ProvenanceMaterial(name=name, digest=digest))

    return builder, build_id, source_digest, tuple(materials)
...
class ProvenanceStore:
    ...
    def add(
        ...
        repeat of the exact same normalized content (HTTP 200).

        Raises :class:`ProvenanceValidationError` for an invalid payload,
        or :class:`ProvenanceError` with code ``provenance_conflict`` when
        different provenance is already recorded; the stored record is
        never overwritten.
        """
