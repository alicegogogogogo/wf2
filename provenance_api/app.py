...
#: Process-local admission policies; never persisted.
...
def _handle_policies_get(
    ...
    if record is None:
        return _error(
            ...
            "404 Not Found",
            "policy_not_found",
            "No admission policy is registered for this resource.",
        )
...
def _admission_body_error(environ: dict[str, Any]) -> str | None:
    """Reject an admission POST that declares or carries a request body.

    A missing ``Content-Length`` is the normal, body-less case. An explicit
    length of zero is accepted; any other (non-integer or positive) value
    means the request carries a body and is rejected.
    """
...
def _handle_admission(
    ...
    # The decision is read-only and takes no request body; a declared or
    # carried body is a bad request rather than being silently ignored.
    ...
    severities = [
        record.severity
        for record in vulnerability_store.list_for(raw_id)
    ]
    reasons = evaluate_admission(
        policy,
        state=lifecycle_store.get(raw_id).state,
        has_sbom=sbom_store.get_sbom(raw_id) is not None,
        has_license=license_record is not None,
        spdx_id=license_record.spdx_id if license_record is not None else None,
    ...
    return _json_response(
        start_response,
        "200 OK",
        {"id": raw_id, "allowed": not reasons, "reasons": reasons},
        trailing_newline=True,
    )
    ...
    try:
        ...
        if path.startswith("/resources/"):
            ...
            if separator and suffix.endswith("/admission"):
                # Fallback for a separator inside the id segment so the
                # admission handler rejects it without reading any state.
                return _handle_admission(
                    method,
                    environ,
