...
def _handle_advisories(
    ...
    # The view is read-only and accepts no request body: a declared positive
    # or malformed Content-Length is rejected before any business data is
    # read. An omitted header (and the empty-string CONTENT_LENGTH real WSGI
    # servers seed for a bodyless GET) or an explicit Content-Length of 0 is
    # an empty body and is allowed.
    ...
    # Read-only, computed on the fly. Alerts are walked in global submission
    # order so advisory groups expand by first appearance, which is the
    # commit time of the earliest matching alert and is unrelated to resource
    # registration order. Counts are record totals (no dedup or merging),
    # while affected resources are deduplicated and rendered in resource
    # registration order afterwards.
    resource_ids_by_advisory: dict[str, set[str]] = {}
    counts: dict[str, int] = {}
    max_by_advisory: dict[str, str] = {}
    order: list[str] = []
    for resource_id, alert in vulnerability_store.list_all():
        if severity is not None and alert.severity != severity:
            continue
        if alert.advisory not in counts:
            counts[alert.advisory] = 0
            resource_ids_by_advisory[alert.advisory] = set()
            order.append(alert.advisory)
        counts[alert.advisory] += 1
        resource_ids_by_advisory[alert.advisory].add(resource_id)
        current = max_by_advisory.get(alert.advisory)
        max_by_advisory[alert.advisory] = (
            alert.severity
            if current is None
            else max_severity([current, alert.severity])
        )

    registration_order = [resource.id for resource in store.list_all()]
    summaries: list[dict[str, object]] = []
    for advisory in order:
        affected = resource_ids_by_advisory[advisory]
        summaries.append(
            {
                "advisory": advisory,
                "affected_resources": [
                    resource_id
                    for resource_id in registration_order
                    if resource_id in affected
                ],
                "advisory_count": counts[advisory],
                "max_severity": max_by_advisory[advisory],
            }
        )
