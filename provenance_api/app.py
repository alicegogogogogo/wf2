...
from .risk import calculate_score, risk_level
...
def _handle_risk(
    ...
    # The score is computed on demand from the current in-process state
    # and is never itself recorded.
    license_record = sbom_store.get_license(raw_id)
    policy = policy_store.get(raw_id)
    score = calculate_score(
        ...
        license_allowlist=(
            tuple(policy.license_allowlist) if policy is not None else None
        ),
    )
...
def _handle_notifications_post(
    ...
    # Validate the whole body before touching any store, so a bad request
    # can never leave a partial notification record.
    try:
    ...
    record = notification_store.add(raw_id, channel, target, message)
    return _json_response(
        start_response,
        "201 Created",
        record.to_dict(),
    ...
    try:
        ...
        if path.startswith("/resources/"):
            ...
            if separator and suffix.endswith("/notifications"):
                # Same fallback so a separator in the id is rejected
                # without recording any notification.
                return _handle_notifications(
                    method,
                    environ,
