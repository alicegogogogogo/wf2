...
这是一个面向代码、AI 模型、数据集和构建产物的后端服务基线。当前版本提供可运行的 HTTP 服务、健康检查、进程内的资源登记与查询、资源之间的依赖关系登记、拓扑查询与影响分析接口、按资源标识提交原始字节的内容校验接口、内容寻址的分块存储、组装与成品读取接口、分块上传会话状态查询接口、资源生命周期状态（晋级、撤回与隔离）接口、按资源登记与查询安全告警（漏洞）的接口、按资源登记与查询 SBOM 文档与许可证声明的接口、按资源登记与查询构建来源证明（provenance）的接口、按资源登记与查询准入策略并执行只读准入评估的接口、按资源即时计算风险评分的接口、按资源登记与查询通知记录的接口，全局镜像层缓存的写入、读取与状态查询接口，以及全局镜像源的登记、查询与按摘要从镜像源拉取镜像层的接口。
...
- 路径摘要不是 64 位十六进制（此时不读取请求体）；
- `Content-Type` 不是 `application/octet-stream`；
- `Content-Length` 缺失、取值非法或实际字节不足；
- 携带任意查询参数。

内容字节与路径摘要不一致时返回 HTTP 409（错误码 `digest_mismatch`），不写入任何缓存条目；同一摘要已缓存不同字节时返回 HTTP 409（错误码 `cache_conflict`），保留原条目；写入会使已用字节超过配额时返回 HTTP 409（错误码 `cache_quota_exceeded`），缓存保持原样。

对 `/cache/layers/{digest}` 使用 `GET`、`POST` 之外的方法，或对 `/cache/status` 使用 `GET` 之外的方法，都返回 HTTP 405（错误码 `method_not_allowed`，响应带 `Allow` 头）且不改状态。跨层批量操作不在本接口范围内。

## 镜像源与按摘要拉取

镜像源是一个命名的、全局唯一的上游 HTTP(S) 地址，可按摘要回源取回镜像层。镜像源登记、回源拉取都复用上面的全局镜像层缓存：拉取先查缓存，未命中才回源，校验通过后写入缓存。镜像源记录只保存在当前进程内存中，重启即清空，不写入任何文件；本次不提供删除、清空与更新。

### 登记镜像源：`POST /mirrors`

请求体必须是一个完整的 JSON 对象，只允许以下两个字段，二者均为必填：

| 字段 | 类型 | 是否必填 | 说明 |
| --- | --- | --- | --- |
| `name` | string | 是 | 镜像源名称，非空，全局唯一（区分大小写，原样回显） |
...
curl -s -X POST http://127.0.0.1:8000/mirrors \
  -H 'Content-Type: application/json' \
  -d '{"name":"hub-a","upstream":"https://mirror.example.invalid/"}'
```

```json
{"id":"…","name":"hub-a","upstream":"https://mirror.example.invalid/"}
```

名称重复登记返回 HTTP 409（错误码 `duplicate_mirror`），原镜像源（含其上游地址）不被覆盖或合并。

### 列出镜像源：`GET /mirrors`

按登记顺序返回全部镜像源，空集合也是成功响应，响应为紧凑 JSON 并以换行结束：
...
返回该镜像源，回显 `id`、`name`、`upstream` 三项，形状与登记响应相同。标识格式合法但镜像源不存在时返回 HTTP 404（错误码 `mirror_not_found`），不改变任何状态。

### 拉取镜像层：`POST /mirrors/{id}/pull/{digest}`

`{digest}` 必须是 64 位十六进制摘要（大小写均可，比较时统一按小写规范化）。拉取流程：

1. 先查全局 `/cache`：命中时直接返回该层的**原始缓存字节**，不联系上游；
2. 未命中时，把登记地址去掉末尾斜杠后拼接 `/layers/{摘要}`（即 `<upstream 去末尾斜杠>/layers/<digest>`）向上游取回该层字节；
3. 对取回字节计算 SHA-256，与路径摘要逐字一致时才写入缓存并返回内容。

成功一律返回 HTTP 200，响应体只包含该层原始字节（无 JSON 包装、无末尾换行），`Content-Type: application/octet-stream`，`Content-Length` 为准确长度，与成品、缓存读取的字节响应一致。**拉取（无论命中、未命中回源、成功或失败）都不改变缓存的命中、未命中计数。**
...
- 上游不可达（连接失败、解析失败等）或返回非 HTTP 200 时，返回 HTTP 502（错误码 `mirror_fetch_failed`），不写缓存、不改计数。
- 上游字节的 SHA-256 与请求摘要不符时，返回 HTTP 502（错误码 `mirror_digest_mismatch`），不写缓存。
- 写入会使已用字节超过缓存配额时，返回 HTTP 409（错误码 `cache_quota_exceeded`），缓存保持不变；配额由启动参数 `--cache-quota` 控制。

### 镜像源接口的错误

下列情况都返回 HTTP 400（错误码 `invalid_request`），且不留下任何镜像源记录、不改变缓存：

- 登记请求体缺失、不是合法 UTF-8 JSON，或顶层不是 JSON 对象；
- 缺少 `name`/`upstream`、出现未知字段、任一字段类型错误或为空；
- `upstream` 不是 `http` 或 `https` 的绝对地址；
- 路径标识为空或含 `/`、`\\`，或拉取摘要不是 64 位十六进制（或缺摘要段）；
- 上述任意路径携带查询参数。

标识合法但镜像源不存在时，单条查询与拉取都返回 HTTP 404（错误码 `mirror_not_found`），且不改变状态。方法限制：

- `/mirrors` 仅允许 `GET`、`POST`（`Allow: GET, POST`）；
- `/mirrors/{id}` 仅允许 `GET`（`Allow: GET`）；
- `/mirrors/{id}/pull/{digest}` 仅允许 `POST`（`Allow: POST`）；

其他方法返回 HTTP 405（错误码 `method_not_allowed`，响应带 `Allow` 头）。本次不改动 `/cache` 的三个入口；镜像源的删除、清空与并发处理均不在范围内。
