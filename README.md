...
在分散的依赖查询与影响分析之外，服务提供一个全局图谱出口，把当前进程内的全部资源与直接依赖边一次性给出。图谱即时计算、只读，不落任何记录，也不改变资源、依赖、引用与缓存状态；资源注销、依赖删除或跨仓库引用增删后，下一次请求立即按剩余数据重算。数据仅保存在进程内存中，服务停止或重启即清空，不生成任何持久化文件；启动方式、健康检查与进程内存边界与其他接口一致。

### 整张图谱：`GET /graph`

不带任何查询参数即取回整张图。响应顶层固定为 `nodes` 与 `edges` 两段，紧凑 UTF-8 JSON，以单个换行结束：
...
```json
{"nodes":[{"id":"…","name":"model-a","category":"model","digest":"…"}],"edges":[{"resource_id":"<起点 id>","dependency_id":"<被依赖资源 id>"}]}
```

- 节点按资源登记顺序展开；每个节点只含 `id`、`name`、`category`、`digest` 四项，键序固定（不返回 `source`）。
- 边按起点资源的登记顺序分组，组内按各条关系的建立先后展开；每条边只含 `resource_id` 与 `dependency_id` 两项，分别指向起点与被依赖的资源。人工登记的边与跨仓库解析生成的边完全同构，统一排在其中。
- 边只表示直接依赖，不展开间接可达关系；间接闭包仍由既有依赖查询与影响分析给出。
- 没有任何资源时返回 `{"nodes":[],"edges":[]}`，仍是 HTTP 200。

### 一跳邻域：`GET /graph?focus={id}`

可选查询参数 `focus` 指定一个资源，只返回它的一跳邻域：节点段只有焦点资源自身一条（四项同上），边段包含它的全部直接出边与直接入边。

- 出边排在前面，按建立先后排列；入边排在后面，顺序与整张图谱一致（按起点资源登记顺序分组、组内按建边先后）。
- 不包含间接可达的资源，也不包含邻域节点之间的边；邻域资源只在边中以标识出现，不返回它们的节点记录。
- 焦点资源没有任何邻边时，边段为空数组（节点段仍是焦点自身）；这与空集合的口径一致，仍是 HTTP 200。
...
- `focus` 值为空、或含路径分隔符（`/`、`\\`）时返回 HTTP 400（错误码 `invalid_request`），且不读取业务数据。
- 出现未知查询参数、或重复的 `focus` 参数时返回 HTTP 400（错误码 `invalid_request`）。
- 请求声明了非空请求体（含非法的 `Content-Length`）时返回 HTTP 400（错误码 `invalid_request`）；显式 `Content-Length: 0` 视为空体。
- `focus` 格式合法但指向的资源不存在时返回 HTTP 404（错误码 `resource_not_found`），不改动任何状态。
- `GET` 之外的方法返回 HTTP 405（错误码 `method_not_allowed`），`Allow` 头只给 `GET`。
- 任何请求都不改变资源、依赖、引用、缓存或其他既有状态，也不改变资源登记、依赖拓扑与影响分析等既有入口的响应与错误码。
