...
这是一个面向代码、AI 模型、数据集和构建产物的后端服务基线。当前版本提供可运行的 HTTP 服务、健康检查、进程内的资源登记与查询、资源之间的依赖关系登记、拓扑查询与影响分析接口、按资源标识提交原始字节的内容校验接口、内容寻址的分块存储、组装与成品读取接口、分块上传会话状态查询接口、资源生命周期状态（晋级、撤回与隔离）接口、按资源登记与查询安全告警（漏洞）的接口、按资源登记与查询 SBOM 文档与许可证声明的接口、按资源登记与查询构建来源证明（provenance）的接口，以及按资源登记、查询准入策略并执行只读准入评估的接口。
...
可以为已登记的资源登记一份准入策略，并按资源执行一次只读的准入判定。策略只保存在当前进程内存中，服务停止或重启后随资源一起清空，不会写入任何文件；本次不校验签名，也不改变任何晋级条件。每个资源至多保留一份策略。
...
| 字段 | 类型 | 是否必填 | 说明 |
| --- | --- | --- | --- |
| `name` | string | 是 | 非空策略名称，最多 256 个 Unicode 码点 |
| `evidence_requirements` | array | 是 | 证据要求，只允许 `provenance`、`sbom`、`license`，区分大小写；不得重复，可以为空数组 |
| `allowed_licenses` | array | 是 | SPDX 许可证标识数组，元素为非空字符串且不得重复；可以为空数组（空数组表示不限制许可证） |
| `max_severity` | string | 是 | 级别上限，取 `critical`、`high`、`medium`、`low` 之一，比较时忽略大小写，按小写保存与输出 |

不允许额外未知字段，也不接受空值。首次登记成功返回 HTTP 201，响应体为紧凑 UTF-8 JSON，键序固定（`id`、`name`、`evidence_requirements`、`allowed_licenses`、`max_severity`）并以换行结束：

```bash
curl -s -X POST http://127.0.0.1:8000/resources/$ID/policies \
  -H 'Content-Type: application/json' \
  -d '{"name":"release-gate","evidence_requirements":["provenance","sbom","license"],"allowed_licenses":["Apache-2.0","MIT"],"max_severity":"HIGH"}'
```

```json
{"id":"…","name":"release-gate","evidence_requirements":["provenance","sbom","license"],"allowed_licenses":["Apache-2.0","MIT"],"max_severity":"high"}
```

幂等与冲突：

- 规范化后的内容完全相同（级别上限忽略大小写）再次提交视为幂等，返回 HTTP 200，回显内容与首次登记相同，且不生成第二份记录。
- 已有策略时提交任何不同内容（名称、证据要求、许可证清单或级别上限任一不同，含元素增删或顺序不同）返回 HTTP 409（错误码 `policy_conflict`），原策略保持不变。

### 查询策略：`GET /resources/{id}/policies`

返回该资源唯一一份准入策略，形状与登记响应相同。资源存在但从未登记策略时返回 HTTP 404（错误码 `policy_not_found`）。

### 准入评估：`POST /resources/{id}/admission`

对资源执行一次只读判定，**不接受请求体**，也不接受查询参数。成功返回 HTTP 200，响应体为紧凑 UTF-8 JSON，键序固定（`id`、`allowed`、`reasons`）并以换行结束：
...
| --- | --- |
| `id` | 被判定资源的标识 |
| `allowed` | 全部检查通过为 `true`；命中任一拒绝原因为 `false` |
| `reasons` | 命中的原因代码数组，按固定顺序排列；全部通过时为空数组 `[]` |

判定按下列顺序执行，原因代码也按该顺序输出（状态、证据、许可证、严重度）：

1. 资源处于 `withdrawn`（撤回）或 `quarantined`（隔离）状态时直接拒绝，原因为 `state_blocked`，**不再检查其他任何条件**。
2. 证据要求逐项检查：缺少 SBOM 文档为 `no_sbom`、缺少许可证声明为 `no_license`、缺少构建来源证明为 `no_provenance`，按此顺序各出现至多一次。
3. 策略的许可证清单非空、而资源实际许可证的 `spdx_id` 不在清单中（或资源未登记许可证）时拒绝，原因为 `license_denied`；清单为空时不作许可证限制。
4. 资源存在严重度高于上限的安全告警时拒绝，原因为 `severity_exceeded`；级别比较忽略大小写，处于上限或低于上限不算命中，该原因至多出现一次。

`reasons` 可能同时包含多条原因（除 `state_blocked` 单独出现外），例如证据、许可证与严重度同时不满足时：

```json
{"id":"…","allowed":false,"reasons":["no_sbom","no_license","no_provenance","license_denied","severity_exceeded"]}
```

评估只读取生命周期状态、SBOM、许可证、构建来源证明与安全告警，不写入或修改任何状态；无论判定结果如何都可以安全重复调用。准入评估不改变晋级条件。
...
- 缺少 `name`、`evidence_requirements`、`allowed_licenses`、`max_severity` 中任一字段，或出现未知字段；
- `name` 不是字符串、为空或超过 256 个 Unicode 码点；
- `evidence_requirements` 不是数组、元素不是 `provenance`/`sbom`/`license` 之一，或出现重复；
- `allowed_licenses` 不是数组、元素不是非空字符串，或出现重复；
- `max_severity` 不是四个合法级别之一；
- 路径标识为空或含 `/`、`\\`；登记、查询与评估请求携带任意查询参数；
- 准入评估请求携带任何请求体。

路径标识格式合法但资源不存在时，登记、查询与评估都返回 HTTP 404（错误码 `resource_not_found`）；资源存在但尚未登记策略时，查询与评估返回 HTTP 404（错误码 `policy_not_found`）。对 `/resources/{id}/policies` 使用 `GET`、`POST` 之外的方法返回 HTTP 405（错误码 `method_not_allowed`，`Allow: GET, POST`）；对 `/resources/{id}/admission` 使用 `POST` 之外的方法返回 HTTP 405（`Allow: POST`）。

任何失败都不会写入策略，也不会修改资源、依赖、内容、游标、生命周期状态、安全告警、SBOM 文档、许可证声明或构建来源证明；策略与判定结果仅存于当前进程内存，停止或重启即清空，不生成任何文件，本次不校验签名也不改变晋级条件。
...
- 当前公开业务接口为健康检查、上述资源登记/查询接口、资源依赖关系登记、依赖拓扑查询与影响分析接口、资源内容校验接口、分块上传、组装、分块上传会话状态查询与成品读取接口，资源生命周期状态读取与提交接口，安全告警（漏洞）的登记与查询接口，SBOM 文档与许可证声明的登记与查询接口，构建来源证明（provenance）的登记与查询接口，以及准入策略的登记、查询接口与按资源的只读准入评估接口；资源、依赖、分块会话、成品内容、生命周期状态、安全告警、SBOM 文档、许可证声明、构建来源证明与准入策略均仅存于进程内存，不承诺跨进程或重启后的保存。
