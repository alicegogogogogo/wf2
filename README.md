...
这是一个面向代码、AI 模型、数据集和构建产物的后端服务基线。当前版本提供可运行的 HTTP 服务、健康检查、进程内的资源登记与查询、资源之间的依赖关系登记、拓扑查询与影响分析接口、按资源标识提交原始字节的内容校验接口、内容寻址的分块存储、组装与成品读取接口、分块上传会话状态查询接口、资源生命周期状态（晋级、撤回与隔离）接口、按资源登记与查询安全告警（漏洞）的接口、按资源登记与查询 SBOM 文档与许可证声明的接口、按资源登记与查询构建来源证明（provenance）的接口、按资源登记与查询准入策略并执行只读准入评估的接口、按资源即时计算风险评分的接口、按资源登记与查询通知记录的接口，以及全局镜像层缓存的写入、读取、状态查询、单层删除与清空接口，以及全局镜像源的登记、查询与按摘要通过镜像源拉取镜像层的接口，以及把本地资源指向其他仓库资源、登记时即时解析并建立依赖边的跨仓库引用接口，以及按资源登记、查询内容签名并只读验签的接口。
...
可以为已登记的资源登记一份内容签名记录，并通过只读验签接口即时复核。签名记录只保存在当前进程内存中，服务停止或重启后随资源一起清空，不会写入任何文件。每个资源至多保留一条签名记录。

### 登记签名：`POST /resources/{id}/signatures`

请求体必须是一个完整的 JSON 对象，且只允许以下五个字段，全部必填：

| 字段 | 类型 | 说明 |
| --- | --- | --- |
| `signer` | string | 签名方，非空文本，最多 256 个 Unicode 码点 |
| `algorithm` | string | 签名算法，只允许 `hmac-sha256` 与 `hmac-sha512`（区分大小写） |
| `key_id` | string | 密钥标识，非空文本，最多 256 个 Unicode 码点 |
| `signature` | string | 签名值，十六进制文本；`hmac-sha256` 为 64 位、`hmac-sha512` 为 128 位，接受大小写混合，统一按小写保存与返回 |
| `digest` | string | 被签名的摘要，必须是 64 位十六进制值；接受大小写混合，统一按小写保存与返回 |

签名值按如下规则计算：对 `digest` 的小写文本做 HMAC（`hmac-sha256` 用 SHA-256、`hmac-sha512` 用 SHA-512），密钥取 `key_id` 的 UTF-8 字节，输出小写十六进制。签名长度与算法不符即为格式非法。

同一资源首次登记成功返回 HTTP 201，响应体为紧凑 UTF-8 JSON，键序固定（`id` 在前、`signer`、`algorithm`、`key_id`、`signature`、`digest` 随后）并以换行结束：

```bash
curl -s -X POST http://127.0.0.1:8000/resources/$ID/signatures \
  -H 'Content-Type: application/json' \
  -d '{"signer":"release-bot","algorithm":"hmac-sha256","key_id":"key-1","signature":"…64 位十六进制…","digest":"…64 位十六进制…"}'
```

```json
{"id":"…","signer":"release-bot","algorithm":"hmac-sha256","key_id":"key-1","signature":"…","digest":"…"}
```

幂等与冲突：

- 规范化后的内容完全相同（五个字段逐一一致，十六进制值按小写比较）再次登记视为幂等，返回 HTTP 200，回显内容与首次登记相同，且不生成第二条记录。
- 已有不同签名记录时返回 HTTP 409（错误码 `signature_conflict`），原记录不被覆盖。
- 登记体的 `digest` 与资源登记摘要不一致时返回 HTTP 409（错误码 `signed_digest_mismatch`），不写入任何记录。

### 查询签名：`GET /resources/{id}/signatures`

返回该资源唯一一条签名记录，形状与登记响应相同，仍是 HTTP 200。资源存在但从未登记签名时返回 HTTP 404（错误码 `signature_not_found`）。

### 只读验签：`POST /resources/{id}/signatures/verify`

按登记记录中的 `algorithm`、`key_id`、`digest` 即时重算签名并与登记值比对，不读取请求体、不留下任何记录。响应为 HTTP 200，包含 `id` 与 `valid` 并以换行结束：一致时 `valid` 为 `true`，不一致为 `false`。
...
下列情况都返回 HTTP 400（错误码 `invalid_request`），且不写入或修改任何签名记录或其他状态：

- 请求体缺失、不是合法 UTF-8、无法解码为 JSON，或顶层不是 JSON 对象；
- 缺少五个字段中任一字段，或出现未知字段；
- `signer`、`key_id` 不是字符串、为空或超过 256 个 Unicode 码点；
- `algorithm` 不是 `hmac-sha256` 或 `hmac-sha512`；
- `signature` 不是与算法长度相符的十六进制文本；`digest` 不是 64 位十六进制文本；
- 路径标识为空或含 `/`、`\\`；请求携带任意查询参数。

路径标识格式合法但资源不存在时，三个端点都返回 HTTP 404（错误码 `resource_not_found`）；资源存在但尚未登记签名时，查询与验签返回 404（`signature_not_found`）。对 `/resources/{id}/signatures` 使用 `GET`、`POST` 之外的方法返回 HTTP 405（`Allow: GET, POST`）；对 `/resources/{id}/signatures/verify` 使用 `POST` 之外的方法返回 HTTP 405（`Allow: POST`），错误码均为 `method_not_allowed`。

任何失败都不会写入签名记录，也不会修改资源、依赖、内容、游标、生命周期状态、安全告警、SBOM 文档、许可证、构建来源证明、准入策略、通知、缓存、镜像源或跨仓库引用；验签即时计算不留记录，签名记录仅存于当前进程内存，停止或重启即清空。
...
- 当前公开业务接口为健康检查、上述资源登记/查询接口、资源依赖关系登记、依赖拓扑查询与影响分析接口、资源内容校验接口、分块上传、组装、分块上传会话状态查询与成品读取接口，资源生命周期状态读取与提交接口，安全告警（漏洞）的登记与查询接口，SBOM 文档与许可证声明的登记与查询接口，构建来源证明（provenance）的登记与查询接口，准入策略的登记与查询接口和只读准入评估接口，按资源即时计算的风险评分接口，以及通知记录的登记与查询接口，以及全局镜像层缓存的写入、读取、状态查询、单层删除与清空接口，以及全局镜像源的登记、查询和通过镜像源按摘要拉取镜像层的接口，以及把本地资源指向其他仓库资源、登记时即时解析远端元数据并建立依赖边的跨仓库引用登记与查询接口，以及按资源登记、查询内容签名并只读验签的接口；资源、依赖、分块会话、成品内容、生命周期状态、安全告警、SBOM 文档、许可证声明、构建来源证明、准入策略、通知记录、镜像源登记、跨仓库引用、签名记录与缓存条目及计数均仅存于进程内存（跨仓库引用解析失败不留引用、不建边、不生成本地资源；缓存的删除与清空只移除条目并回落已用字节，不改动资源、依赖、成品、镜像源、跨仓库引用等既有状态，也不重置命中与未命中计数），准入评估结果与风险评分均即时计算、不落记录、不承诺跨进程或重启后的保存。
