...
这是一个面向代码、AI 模型、数据集和构建产物的后端服务基线。当前版本提供可运行的 HTTP 服务、健康检查、进程内的资源登记与查询、资源之间的依赖关系登记、拓扑查询与影响分析接口、按资源标识提交原始字节的内容校验接口、内容寻址的分块存储、组装与成品读取接口、分块上传会话状态查询接口、资源生命周期状态（晋级、撤回与隔离）接口、按资源即时计算晋级阻塞明细的只读视图接口、按资源登记、查询与删除安全告警（漏洞）的接口、把全部告警按公告编号汇总的全局只读视图接口、按资源登记与查询 SBOM 文档与许可证声明的接口、按资源登记与查询构建来源证明（provenance）的接口、按资源登记与查询准入策略并执行只读准入评估的接口、按资源登记、查询与删除安全告警豁免（漏洞例外）并按豁免口径提供只读准入预览的接口、按资源即时计算风险评分的接口、按资源登记与查询通知记录的接口，以及在组件级漏洞关联视图之外、按资源即时计算只读的组件修复建议视图的接口，以及全局镜像层缓存的写入、读取、状态查询、单层删除与清空接口，以及全局镜像源及其签名策略的登记、查询与删除，和按摘要通过镜像源拉取镜像层、按镜像源对登记上游发起探活并读取最近一次探活结果的接口，以及把本地资源指向其他仓库资源、登记时即时解析并建立依赖边的跨仓库引用接口，以及按资源登记、查询内容签名并对登记签名执行只读验签的接口。
...
可以对已登记的资源逐条登记安全告警，也可以通过批量入口一次提交一组，或按登记时返回的告警标识删除单条告警。告警只保存在当前进程内存中，服务停止或重启后随资源一起清空，不会写入任何文件，也不承诺跨进程同步。告警按提交顺序保存；更新与并发处理不在当前范围内。
...
按登记时返回的告警标识删除该资源下的单条告警。只接受 `DELETE` 方法；请求不带请求体（声明正长度或畸形的 `Content-Length` 返回 HTTP 400，显式 `Content-Length: 0` 视为空请求体），也不接受任何查询参数。成功返回 HTTP 200，并回显被删告警的完整内容，字段与单条登记响应完全一致（键序固定为 `id`、`advisory`、`component`、`severity`、`summary`、`fixed_version`），正文仍是紧凑 UTF-8 JSON，以单个换行结束：
...
删除立即生效并同步反映到所有既有视图，无需重启或额外刷新：

- 被删记录立即从单条查询与 `severity` 严重度筛选结果中消失，不再出现在提交序列里；
- 准入评估与准入预览的超限严重度判定、风险评分都不再计入这条告警（豁免计数同样不再包含它）；
- 组件漏洞关联（组件风险视图）与组件修复建议两处视图同步移除这条告警的关联与计数；
- 全局公告汇总（`GET /advisories`）即时重算：该公告的计数与受影响资源列表随之收缩；计数归零、不再有任何告警的公告直接从汇总中消失，其余公告的首次出现顺序不变；
- 依赖漏洞传导等其他按告警即时计算的视图同样不再计入这条告警。

已登记的告警豁免不会因删除而撤回：豁免记录原样保留，只是暂时失去匹配对象，不再计入准入预览的豁免条数；在没有匹配告警期间重新登记同公告与组件的豁免仍会按既有口径返回 HTTP 404（错误码 `vulnerability_not_found`）。同一公告编号与组件的告警在同一资源下再次登记后，既有豁免立即重新生效，无需重复登记；删除告警也会释放该资源上的公告/组件去重键，相同公告与组件可以立即重新登记。

告警标识未知、属于其他资源，或此前已经删除时，都返回 HTTP 404（错误码 `vulnerability_not_found`），三种情况无法从响应区分，且都不改动任何已有记录。重复删除同一告警只会得到 404，不会影响同一公告下的其他告警。

路径上的资源标识或告警标识为空、含 `/`、`\\` 时返回 HTTP 400（错误码 `invalid_request`）；携带任意查询参数、或声明请求体时同样返回 HTTP 400，且不读取任何业务数据。资源标识格式合法但资源不存在返回 HTTP 404（错误码 `resource_not_found`）。对该路径使用 `DELETE` 之外的方法返回 HTTP 405（错误码 `method_not_allowed`，`Allow: DELETE`）。

任何失败都不会新增、修改或删除任何告警，也不改变豁免、准入、风险、资源等既有状态。单条登记、批量登记与查询的既有行为完全不变，校验口径也保持一致；告警数据仍只存于进程内存，停止或重启即清空，服务不生成任何文件。
...
删除单条告警（`DELETE /resources/{id}/vulnerabilities/{vulnerability-id}`）的错误口径见上一节：路径标识为空或含 `/`、`\\`、携带任意查询参数或声明请求体均为 HTTP 400（错误码 `invalid_request`）；告警标识未知、属于其他资源或已删除均为 HTTP 404（错误码 `vulnerability_not_found`）；`DELETE` 之外的方法返回 HTTP 405（错误码 `method_not_allowed`，`Allow: DELETE`）。
