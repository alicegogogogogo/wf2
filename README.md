...
这是一个面向代码、AI 模型、数据集和构建产物的后端服务基线。当前版本提供可运行的 HTTP 服务、健康检查、进程内的资源登记与查询、资源之间的依赖关系登记、拓扑查询与影响分析接口、按资源标识提交原始字节的内容校验接口、内容寻址的分块存储、组装与成品读取接口、分块上传会话状态查询接口、资源生命周期状态（晋级、撤回与隔离）接口，以及按资源登记与查询安全告警（漏洞公告）的接口。
...
可以对已登记的资源登记安全告警。告警只保存在当前进程内存中，服务停止或重启后随资源一起清空，不生成任何文件，也不承诺跨进程同步；批量登记、更新、删除与并发控制不在本次范围内。
...
| 字段 | 类型 | 是否必填 | 说明 |
| --- | --- | --- | --- |
| `advisory_id` | string | 是 | 非空公告编号，最多 256 个 Unicode 码点 |
| `component` | string | 是 | 非空组件名称，最多 256 个 Unicode 码点 |
| `severity` | string | 是 | `critical`、`high`、`medium`、`low` 之一，大小写无关，按小写存储 |
| `summary` | string | 是 | 非空摘要，最多 2048 个 Unicode 码点 |
| `fix_version` | string | 否 | 非空修复版本，最多 256 个 Unicode 码点；省略时为 `null`，显式给 `null` 非法 |

不允许额外未知字段。成功时返回 HTTP 201，告警按提交顺序保存，响应依次包含服务生成的稳定标识 `id` 与五项业务字段，紧凑 UTF-8 JSON、键序稳定并以换行结束：

```bash
curl -s -X POST http://127.0.0.1:8000/resources/$ID/vulnerabilities \
  -H 'Content-Type: application/json' \
  -d '{"advisory_id":"CVE-2026-0001","component":"widget","severity":"HIGH","summary":"存在可远程触发的缺陷","fix_version":"1.2.3"}'
```

```json
{"id":"…","advisory_id":"CVE-2026-0001","component":"widget","severity":"high","summary":"存在可远程触发的缺陷","fix_version":"1.2.3"}
```

省略 `fix_version` 时该字段返回 `null`。同一资源下公告编号与组件均相同的重复登记返回 HTTP 409（错误码 `duplicate_vulnerability`），原告警（含级别、摘要与修复版本）保持不变；相同公告编号但组件不同，或同一对公告编号与组件登记在不同资源下，都视为不同告警。
...
结果按提交顺序排列，每个告警只出现一次；该资源没有任何告警时返回空数组 `[]`，仍是 HTTP 200。

可选查询参数 `severity` 取 `critical`、`high`、`medium`、`low` 之一，比较时忽略大小写（如 `severity=HIGH`），响应中的级别仍按小写输出；筛选结果保持提交顺序，没有命中时返回空数组。

### 告警接口的错误

下列情况都返回 HTTP 400（错误码 `invalid_request`），且不留下任何告警记录：

- 请求体缺失、不是合法 UTF-8、无法解码为 JSON，或顶层不是 JSON 对象；
- 缺少必填字段、字段类型错误、必填字段为空、`fix_version` 显式为 `null`/为空/类型错误，或出现未知字段；
- `severity` 不是四个合法级别之一，或文本超过码点上限（公告编号、组件与修复版本 256；摘要 2048）；
- 路径标识为空或含 `/`、`\\`；
- 登记请求携带任意查询参数；查询请求出现未知或重复参数，或 `severity` 为空、非法。

标识格式合法但资源不存在时，登记与查询都返回 HTTP 404（错误码 `resource_not_found`），且不改变任何告警。

对该路径使用 `GET`、`POST` 之外的方法返回 HTTP 405（错误码 `method_not_allowed`，`Allow: GET, POST`）。

任何失败都不会修改资源、依赖、分块、成品、生命周期状态或列表游标；重复的失败请求也不会产生隐藏记录。
...
- 当前公开业务接口为健康检查、上述资源登记/查询接口、资源依赖关系登记、依赖拓扑查询与影响分析接口、资源内容校验接口、分块上传、组装、分块上传会话状态查询与成品读取接口、资源生命周期状态读取与提交接口，以及按资源登记与查询安全告警的接口；资源、依赖、分块会话、成品内容、生命周期状态与安全告警均仅存于进程内存，不承诺跨进程或重启后的保存。
