...
在内容校验之外，可以把一份资源内容拆成若干有序分块上传，由服务按序拼接、计算 SHA-256 并与登记摘要核对，成功后保存为该资源的**成品内容**供读取。上传会话既可以由首个被接受的分块隐式建立，也可以在传字节之前通过显式开传先声明总块数与目标摘要。分块会话、分块字节与成品内容都只保存在当前进程内存中，服务停止或重启后随资源一起清空，不写入任何文件。

### 显式开传：`POST /resources/{id}/chunks/start`

入口挂在分块集合路径之后再加一段 `start`，只接受 `POST` 方法。调用方在传任何字节之前先声明本次上传的总块数与目标摘要，服务在不写入任何分块的前提下完成校验并建立会话。请求体必须是一个完整的 JSON 对象，且只含以下两个字段：

| 字段 | 类型 | 是否必填 | 说明 |
| --- | --- | --- | --- |
| `total_chunks` | number | 是 | 十进制正整数，本次上传的总块数 |
| `digest` | string | 是 | 64 位十六进制目标摘要，接受大小写混合，按小写规范化后与登记摘要比对 |

两项都合法且摘要与登记值一致时返回 HTTP 201，响应体为紧凑 UTF-8 JSON，键序固定（`id`、`digest`、`total_chunks`、`received_chunks`、`missing_chunks`、`complete`、`size`）并以换行结束；已收块数为 `0`，缺失序号按数值升序列全，完成标记为 `false`，字节数为 `null`：
...
- 用相同总块数与目标摘要重复开传返回 HTTP 200，响应按当前会话状态给出，保持幂等，不清掉已收分块。
- 参数不同的重复开传返回 HTTP 409（错误码 `chunk_conflict`），已收分块保持原样。
- 摘要格式合法但与登记摘要不一致时返回 HTTP 409（错误码 `digest_conflict`），不建立会话。
- 成品组装完成后再开传返回 HTTP 409（错误码 `content_already_complete`）。

显式开传之后，会话状态查询按已开始的口径返回 HTTP 200，不再返回尚未开始的冲突结果；后续上传的分块必须与开传声明的总块数和目标摘要一致，不一致按分块上传的既有冲突口径（`chunk_conflict`）拒绝。开传后也可以直接重置这场会话，清掉的块数记 `0`。

错误与边界：

- 请求体缺失、无法解码或顶层不是 JSON 对象，返回 HTTP 400（错误码 `invalid_request`）。
- 字段缺失、类型错误、含未知字段或取值非法（总块数不是正整数、摘要不是 64 位十六进制），同样返回 HTTP 400（错误码 `invalid_request`），且不建立会话。
- 资源标识为空或含 `/`、`\\`，或携带任意查询参数，返回 HTTP 400（错误码 `invalid_request`）。
- 标识合法但资源不存在返回 HTTP 404（错误码 `resource_not_found`）。
- `POST` 之外的方法返回 HTTP 405（错误码 `method_not_allowed`，`Allow: POST`）。
...
首个被接受的分块确定本次上传的总块数和目标摘要（已经显式开传的会话以开传声明为准）；之后每个分块都必须重复相同的两个头部值。分块可以乱序到达，序号也可以跳过，只要组装前全部收齐即可。
...
- 首个分块（或显式开传）之后提交的 `X-Total-Chunks` 或 `X-Content-Digest` 与已确定的声明不一致，返回 HTTP 409（错误码 `chunk_conflict`），不写入本分块。
...
- 资源存在但**从未开始上传**（既未显式开传，也没有任何分块被接受）时返回 HTTP 409（错误码 `chunks_not_started`），且不会因此创建会话。
- 已经开始上传（无论是否收齐、是否组装过，含只显式开传尚未传块）时返回 HTTP 200，响应体为紧凑 UTF-8 JSON，键序固定（`id`、`digest`、`total_chunks`、`received_chunks`、`missing_chunks`、`complete`、`size`）并以换行结束：
...
入口挂在分块上传路径去掉序号的那一段上（即分块集合本身），只接受 `DELETE` 方法。请求不得携带任何查询参数，也不得声明非空请求体（显式 `Content-Length: 0` 视为空体）。重置把该资源已收分块、会话确定的总块数与目标摘要绑定一并清掉，会话整体回到**尚未开始**的状态：不留下半条记录。之后可以用不同的总块数与目标摘要重新开传，序号依旧从 `0` 开始收齐；新目标摘要仍须与资源登记摘要一致，不一致仍按分块上传的既有口径（`digest_conflict`）拒绝写入。
...
- 资源存在但**从未开始上传**（既未显式开传，也没有任何分块被接受）时返回 HTTP 409（错误码 `chunks_not_started`），不改动任何状态。
- 只显式开传、尚未收到任何分块的会话可以直接重置，清掉的块数记 `0`。
