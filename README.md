...
这是一个面向代码、AI 模型、数据集和构建产物的后端服务基线。当前版本提供可运行的 HTTP 服务、健康检查、进程内的资源登记与查询、资源之间的依赖关系登记、拓扑查询与影响分析接口、按资源标识提交原始字节的内容校验接口、内容寻址的分块存储、组装与成品读取接口、分块上传会话状态查询接口、资源生命周期状态（晋级、撤回与隔离）接口、按资源登记与查询安全告警（漏洞）的接口、按资源登记与查询 SBOM 文档与许可证声明的接口、按资源登记与查询构建来源证明（provenance）的接口、按资源登记与查询准入策略并执行只读准入评估的接口、按资源即时计算风险评分的接口、按资源登记与查询通知记录的接口，以及全局镜像层缓存的写入、读取、状态查询、单层删除与清空接口，以及全局镜像源的登记、查询与按摘要通过镜像源拉取镜像层的接口，以及跨仓库引用的登记、解析与查询接口。
...
可以登记若干镜像源，并通过某个镜像源按摘要拉取镜像层。镜像源记录只保存在当前进程内存中，重启即清空，不写入任何文件；拉取复用全局 `/cache` 层缓存：命中既有缓存时直接返回原始字节，未命中时向登记的上游地址取回、校验摘要后写入缓存再返回。镜像源之间相互独立，名称全局唯一。
...
除进程内登记的依赖外，可以为已登记的资源登记一条**跨仓库引用**，把它与远端仓库中的某个资源对应起来。登记引用时，服务会沿引用给出的上游地址解析远端资源元数据、核对摘要，然后**生成或复用**一个本地资源，并在起点资源与该本地资源之间建立一条依赖边。引用记录与解析出的边都只保存在当前进程内存中，服务停止或重启后随资源一起清空，不写入任何文件；批量登记与并发处理不在当前范围内。

引用挂在 `/resources/{id}/cross-references` 下。

### 登记并解析引用：`POST /resources/{id}/cross-references`

请求体必须是一个完整的 JSON 对象，且只允许以下四个字段：

| 字段 | 类型 | 是否必填 | 说明 |
| --- | --- | --- | --- |
| `repository` | string | 是 | 远端仓库名称，非空；同名仓库在本进程内绑定同一个上游地址 |
| `upstream` | string | 是 | 远端仓库上游地址，必须是 `http` 或 `https` 的绝对地址，且不得带查询参数 |
| `remote_id` | string | 是 | 远端资源标识，非空，且不得包含 `/`、`\\` |
| `digest` | string | 是 | 期望的远端资源摘要，必须是 64 位十六进制值，接受大小写混合，统一按小写处理 |

成功解析返回 HTTP 201，响应体为紧凑 UTF-8 JSON，键序固定（`repository`、`upstream`、`remote_id`、`digest`、`resource_id`）并以换行结束，其中 `resource_id` 是路径中的起点资源标识：

```bash
curl -s -X POST http://127.0.0.1:8000/resources/$ID/cross-references \
  -H 'Content-Type: application/json' \
  -d '{"repository":"primary","upstream":"https://repo.example.invalid/","remote_id":"remote-model-1","digest":"aaaa…"}'
```

```json
{"repository":"primary","upstream":"https://repo.example.invalid/","remote_id":"remote-model-1","digest":"aaaa…","resource_id":"<起点资源 id>"}
```

#### 解析过程

1. 校验请求体与路径标识（见下方错误约定），并确认起点资源存在。
2. 同一 `repository` 名称在本进程内只能绑定一个 `upstream`；再次登记同名仓库但上游地址不一致时返回 HTTP 409（错误码 `repository_conflict`），不发起远端请求。`(repository, remote_id)` 相同的重复登记返回 HTTP 409（错误码 `duplicate_reference`），该判断优先于上游地址一致性判断。
3. 沿用镜像源拉取的地址拼接规则：把 `upstream` 去掉末尾斜杠后拼接 `/resources/<remote_id>` 发起 GET，取回远端资源元数据 JSON。
4. 从元数据中读取 `name`、`category`、`digest`、`source` 四个字段并校验；元数据中的 `digest` 必须与请求期望的摘要（小写）逐字一致。
5. 本地已存在同摘要资源时：其 `name`、`category` 与远端元数据一致则**复用**该资源；任一不一致返回 HTTP 409（错误码 `identity_conflict`）。本地不存在同摘要资源时，按远端元数据**生成**一个新的本地资源（`source` 缺省为 `null`）。
6. 在起点资源与该本地资源之间建立一条同向依赖边：边必须经过与人工登记依赖相同的自环与成环检查，且同向边已存在时按既有的重复依赖处理（HTTP 409 `duplicate_dependency`）。新生成的目标资源此前是孤立节点，指向它的边不可能成环。
7. 取回的元数据 JSON 以资源摘要为键写入全局 `/cache` 层缓存：同摘要同字节是幂等空操作，同摘要但字节不同返回 HTTP 409（错误码 `cache_conflict`），超出配额返回 HTTP 409（错误码 `cache_quota_exceeded`）；其余缓存行为与镜像源一致。
8. 以上全部通过后，才一次性提交依赖边与引用记录，并返回 201。

解析出的边与人工登记的边一同参与依赖拓扑（`GET /resources/{id}/dependencies`）与影响分析（`GET /resources/{id}/impact`），排序一律沿用资源登记顺序。

#### 上游与解析错误

下列情况都返回 HTTP 502，且不留引用、不建边、不生成本地资源、不改变缓存：

- 上游连接失败、连接超时，或返回非 200 状态：错误码 `remote_unreachable`；
- 元数据不是合法 UTF-8 JSON、顶层不是对象、缺少 `name`/`category`/`digest` 字段、字段类型错误、`category` 非法、`digest` 不是 64 位十六进制值，或 `source` 给了空值/错误类型：错误码 `resolution_failed`；
- 远端元数据中的摘要与请求期望不符：错误码 `remote_digest_mismatch`。

#### 冲突汇总

- `repository_conflict`（409）：同名仓库绑定了不同上游地址；
- `duplicate_reference`（409）：同一 `(repository, remote_id)` 重复登记；
- `identity_conflict`（409）：本地已有同摘要资源，但名称或类别与远端元数据不一致；
- `dependency_cycle`（409）：解析结果是自环，或该边会使依赖图成环；图保持不变；
- `duplicate_dependency`（409）：起点到目标资源的同向边已存在，按既有重复依赖处理，不重复登记引用；
- `cache_conflict`（409）：缓存中同摘要已有不同字节；`cache_quota_exceeded`（409）：写入元数据会超过缓存配额。

任何失败都不留引用、不建边、不生成本地资源，也不修改资源、依赖、缓存与其他既有状态。

### 查询引用列表：`GET /resources/{id}/cross-references`

按登记顺序返回该资源的全部跨仓库引用，空集合也是成功响应：

```json
{"cross_references":[{"repository":"primary","upstream":"https://repo.example.invalid/","remote_id":"remote-model-1","digest":"aaaa…","resource_id":"<起点资源 id>"}]}
```

每条记录都只含 `repository`、`upstream`、`remote_id`、`digest`、`resource_id` 五个键，顺序与登记响应一致。该接口只读，不触发任何远端解析，也不改变任何状态。

### 跨仓库引用接口的错误

下列情况都返回 HTTP 400（错误码 `invalid_request`），且不发起远端请求、不改变任何状态：

- 请求体缺失、不是合法 UTF-8、无法解码为 JSON，或顶层不是 JSON 对象；
- 缺少四个字段中任一项、出现未知字段、任一字段为空值；
- `repository` 不是字符串或为空白；`remote_id` 为空或含 `/`、`\\`；
- `digest` 不是 64 位十六进制值；
- `upstream` 不是 `http`/`https` 的绝对地址，或带有查询参数；
- 路径标识为空或含 `/`、`\\`；GET、POST 请求携带任意查询参数。

路径标识格式合法但起点资源不存在时，POST 与 GET 都返回 HTTP 404（错误码 `resource_not_found`）。对 `/resources/{id}/cross-references` 使用 `GET`、`POST` 之外的方法返回 HTTP 405（错误码 `method_not_allowed`，`Allow: GET, POST`）。

跨仓库引用与解析生成的本地资源、依赖边及写入缓存的元数据均仅存于当前进程内存，停止或重启即清空，不生成任何文件；删除、更新、批量与并发处理不在当前范围内。
...
- 当前公开业务接口为健康检查、上述资源登记/查询接口、资源依赖关系登记、依赖拓扑查询与影响分析接口、资源内容校验接口、分块上传、组装、分块上传会话状态查询与成品读取接口，资源生命周期状态读取与提交接口，安全告警（漏洞）的登记与查询接口，SBOM 文档与许可证声明的登记与查询接口，构建来源证明（provenance）的登记与查询接口，准入策略的登记与查询接口和只读准入评估接口，按资源即时计算的风险评分接口，以及通知记录的登记与查询接口，以及全局镜像层缓存的写入、读取、状态查询、单层删除与清空接口，以及全局镜像源的登记、查询和通过镜像源按摘要拉取镜像层的接口，以及跨仓库引用的登记、远端解析与查询接口；资源、依赖、分块会话、成品内容、生命周期状态、安全告警、SBOM 文档、许可证声明、构建来源证明、准入策略、通知记录、镜像源登记、跨仓库引用记录与缓存条目及计数均仅存于进程内存（跨仓库引用解析时生成或复用的本地资源与其依赖边同样只存于进程内存，解析失败不留引用、不建边、不生成本地资源；缓存的删除与清空只移除条目并回落已用字节，不改动资源、依赖、成品、镜像源与跨仓库引用等既有状态，也不重置命中与未命中计数），准入评估结果与风险评分均即时计算、不落记录、不承诺跨进程或重启后的保存。
