...
这是一个面向代码、AI 模型、数据集和构建产物的后端服务基线。当前版本提供可运行的 HTTP 服务、健康检查、进程内的资源登记与查询、资源之间的依赖关系登记、拓扑查询与影响分析接口、按资源标识提交原始字节的内容校验接口、内容寻址的分块存储、组装与成品读取接口、分块上传会话状态查询接口、资源生命周期状态（晋级、撤回与隔离）接口、按资源登记与查询安全告警（漏洞）的接口、按资源登记与查询 SBOM 文档与许可证声明的接口、按资源登记与查询构建来源证明（provenance）的接口、按资源登记与查询准入策略并执行只读准入评估的接口、按资源即时计算风险评分的接口、按资源登记与查询通知记录的接口，以及全局镜像层缓存的写入、读取、删除、清空与状态查询接口，以及全局镜像源的登记、查询与按摘要通过镜像源拉取镜像层的接口。
...
使单个缓存层失效。`{digest}` 必须是 64 位十六进制摘要（大小写均可，统一按小写处理）。请求不接受请求体与查询参数：携带任意请求体或查询参数都返回 HTTP 400（错误码 `invalid_request`），且不改变缓存内容与计数。

成功时返回 HTTP 200，响应体是紧凑 UTF-8 JSON 并以换行结束，依次给出被删层摘要、移除字节数和删除后剩余条目数：

```json
{"digest":"012d8243…","size":11,"entries":0}
```

删除后该层占用的字节同步释放，状态查询的条目数与已用字节随之回落；命中、未命中计数与配额不变，释放出的额度立即可用于新层写入。删除不存在或已经删除的摘要都返回 HTTP 404（错误码 `cache_miss`，错误 JSON 形状与读取未命中一致），删除不计为读取，命中、未命中计数均不改变。删除某层后再通过镜像源按同一摘要拉取，仍按既有链路回源取回、校验并重新写入缓存。

### 清空缓存：`DELETE /cache`

清空挂载在缓存根路径下的全部缓存层。请求不接受请求体与查询参数：携带任意请求体或查询参数都返回 HTTP 400（错误码 `invalid_request`）。

成功时返回 HTTP 200，响应体是紧凑 UTF-8 JSON 并以换行结束，给出本次清掉的条目数与释放的字节数：

```json
{"removed":2,"freed_bytes":30}
```

在空缓存上执行清空同样返回 HTTP 200，两个计数都为 0；重复清空不报错。清空只影响缓存条目与已用字节，命中、未命中计数与配额不变，也不改动资源、依赖、成品、镜像源登记等任何既有状态。
...
- 写入层时，路径摘要不是 64 位十六进制（此时不读取请求体）；
- 写入层时，`Content-Type` 不是 `application/octet-stream`；
- 写入层时，`Content-Length` 缺失、取值非法或实际字节不足；
- 删除单层时，路径摘要不是 64 位十六进制（为空、含分隔符等，此时不读取请求体）；
- 删除单层或清空缓存时，携带请求体；
- 任意缓存接口携带查询参数。
...
对 `/cache/layers/{digest}` 使用 `GET`、`POST`、`DELETE` 之外的方法，返回 HTTP 405（错误码 `method_not_allowed`，响应带 `Allow: GET, POST, DELETE` 头）；对 `/cache` 使用 `DELETE` 之外的方法，返回 HTTP 405（响应带 `Allow: DELETE` 头）；对 `/cache/status` 使用 `GET` 之外的方法，返回 HTTP 405（响应带 `Allow: GET` 头）；以上均不改状态。删除单层与清空缓存彼此独立，没有按摘要批量删除多个层的操作，跨层批量操作不在本接口范围内。
...
成功一律返回 HTTP 200，响应体是层的原始字节（无 JSON 包装、无末尾换行），`Content-Type: application/octet-stream`，`Content-Length` 为准确字节数；除首次未命中写入新条目外，拉取不改变缓存的命中、未命中计数。被删除或清空的缓存层在下次按该摘要拉取时按上述未命中链路重新回源取回并写入，回源字节校验与配额判断不会因层曾被缓存过而跳过。
...
镜像源数据与由拉取写入的缓存条目均仅存于当前进程内存，停止或重启即清空，不生成任何文件；缓存层与整个缓存可分别通过 `DELETE /cache/layers/{digest}` 与 `DELETE /cache` 失效（见上文「镜像层缓存」），镜像源本身的删除与清空以及并发处理不在当前范围内。
