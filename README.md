...
这是一个面向代码、AI 模型、数据集和构建产物的后端服务基线。当前版本提供可运行的 HTTP 服务、健康检查、进程内的资源登记与查询、资源之间的依赖关系登记、拓扑查询与影响分析接口、按资源标识提交原始字节的内容校验接口、内容寻址的分块存储、组装与成品读取接口、分块上传会话状态查询接口、资源生命周期状态（晋级、撤回与隔离）接口、按资源登记与查询安全告警（漏洞）的接口、按资源登记与查询 SBOM 文档与许可证声明的接口、按资源登记与查询构建来源证明（provenance）的接口，以及按资源登记与查询准入策略、执行只读准入评估的接口。
...
可以对已登记的资源按其当前状态即时计算一个风险评分。评分是只读、即时的：每次请求都依据该资源当前的安全告警、SBOM、许可证声明、构建来源证明、生命周期状态与准入策略现场计算，评分结果不落任何记录，也不改变任何状态或既有晋级条件。

### 查询风险评分：`GET /resources/{id}/risk`

不接受请求体与查询参数。评分组成如下：

- 每条安全告警按严重度从高到低加 40、25、10 或 5 分（对应 `critical`、`high`、`medium`、`low`），严重度比较忽略大小写；
- SBOM、许可证声明、构建来源证明每缺一份各加 5 分；
- 生命周期状态为 `withdrawn` 或 `quarantined` 时再加 20 分；
- 当资源已登记策略且策略的 `license_allowlist` 非空时，许可证缺失或许可证 SPDX 标识不在清单内再加 10 分；清单为空或资源未登记策略时不检查许可证；
- 总分封顶为 100。

风险等级按封顶后的总分划分：低于 25 为 `low`，低于 50 为 `medium`，低于 75 为 `high`，其余为 `critical`。
...
```json
{"id":"…","score":15,"level":"low"}
```

路径标识为空或含 `/`、`\\`，或请求携带任意查询参数时返回 HTTP 400（错误码 `invalid_request`）。路径标识格式合法但资源不存在时返回 HTTP 404（错误码 `resource_not_found`），状态不变。对该路径使用 `GET` 之外的方法返回 HTTP 405（错误码 `method_not_allowed`，`Allow: GET`）。

## 通知登记与查询

可以对已登记的资源逐条登记通知记录。通知只保存在当前进程内存中，服务停止或重启后随资源一起清空，不会写入任何文件，也不承诺跨进程同步。每次提交独立生成一条通知记录，各条之间互不影响：不去重、不更新、不删除，按提交顺序保存。
...
| 字段 | 类型 | 长度上限 | 说明 |
| --- | --- | --- | --- |
| `channel` | string | 64 个 Unicode 码点 | 通知渠道，非空字符串 |
| `target` | string | 256 个 Unicode 码点 | 通知目标，非空字符串 |
| `message` | string | 2048 个 Unicode 码点 | 通知内容，非空字符串 |

不允许额外未知字段。登记成功返回 HTTP 201，响应体为紧凑 UTF-8 JSON，键序固定（`id`、`channel`、`target`、`message`）并以换行结束：

```bash
curl -s -X POST http://127.0.0.1:8000/resources/$ID/notifications \
  -H 'Content-Type: application/json' \
  -d '{"channel":"email","target":"secops@example.invalid","message":"需要复核该资源"}'
```

```json
{"id":"…","channel":"email","target":"secops@example.invalid","message":"需要复核该资源"}
```

每次成功提交都生成独立的记录标识并追加在该资源的记录末尾，相同内容的重复提交同样产生新记录。
...
每条记录的形状与登记响应相同（`id`、`channel`、`target`、`message`）。

### 通知接口的错误

下列情况都返回 HTTP 400（错误码 `invalid_request`），且不写入任何通知记录，也不改变其他任何状态：

- 请求体缺失、不是合法 UTF-8、无法解码为 JSON，或顶层不是 JSON 对象；
- 缺少任一必填字段、字段类型错误、字段为空值（含空字符串）、出现未知字段；
- `channel`、`target`、`message` 超过各自的 Unicode 码点上限（64、256、2048）；
- 路径标识为空或含 `/`、`\\`；
- 请求携带任意查询参数。

路径标识格式合法但资源不存在时，POST 与 GET 都返回 HTTP 404（错误码 `resource_not_found`），状态不变。对 `/resources/{id}/notifications` 使用 `GET`、`POST` 之外的方法返回 HTTP 405（错误码 `method_not_allowed`，`Allow: GET, POST`）。

任何失败都不会写入通知记录，也不会修改资源、依赖、内容、游标、生命周期状态、安全告警、SBOM 文档、许可证声明、构建来源证明或准入策略；通知仅存于当前进程内存，停止或重启即清空。
...
- 当前公开业务接口为健康检查、上述资源登记/查询接口、资源依赖关系登记、依赖拓扑查询与影响分析接口、资源内容校验接口、分块上传、组装、分块上传会话状态查询与成品读取接口，资源生命周期状态读取与提交接口，安全告警（漏洞）的登记与查询接口，SBOM 文档与许可证声明的登记与查询接口，构建来源证明（provenance）的登记与查询接口，准入策略的登记与查询接口和只读准入评估接口，资源风险评分的即时只读查询接口，以及通知记录的登记与查询接口；资源、依赖、分块会话、成品内容、生命周期状态、安全告警、SBOM 文档、许可证声明、构建来源证明、准入策略与通知记录均仅存于进程内存，准入评估结果与风险评分不落记录、不承诺跨进程或重启后的保存。
