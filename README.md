...
- 对 `/resources/{id}/dependencies` 使用 `GET`、`POST` 之外的方法返回 HTTP 405（错误码 `method_not_allowed`，`Allow: GET, POST`）；对 `/resources/{id}/dependencies/{dependency_id}` 使用 `DELETE` 之外的方法返回 HTTP 405（错误码 `method_not_allowed`，`Allow: DELETE`）；对 `/resources/{id}/impact` 使用 `GET` 之外的方法返回 HTTP 405（`Allow: GET`）。
...
清掉某个资源尚未完成的分块上传会话：已收分块、首个分块确定的总块数与目标摘要绑定一并清除，会话回到未开始状态，只接受 `DELETE` 方法。请求不得携带任何查询参数，也不得声明非空请求体（显式 `Content-Length: 0` 视为空体）。

成功返回 HTTP 200，响应体为紧凑 UTF-8 JSON，键序固定（`id`、`digest`、`removed_chunks`）并以单个换行结束：
...
```json
{"id":"<资源 id>","digest":"<被清除会话的目标摘要>","removed_chunks":2}
```

| 字段 | 说明 |
| --- | --- |
| `id` | 资源标识 |
| `digest` | 被清除会话的目标摘要（小写） |
| `removed_chunks` | 本次清掉的不同序号分块数量 |

- 重置成功后可以用不同的总块数与目标摘要重新开传，序号依旧从零开始收齐；新目标摘要仍须与资源登记摘要一致，不一致仍按既有口径拒绝写入（`digest_conflict`）。
- 资源存在但从未开始上传时返回 HTTP 409（错误码 `chunks_not_started`），不留下任何会话记录。
- 已经组装完成的会话不得重置，返回 HTTP 409（错误码 `content_already_complete`）；成品字节保持原样，读取成品与会话状态查询的既有结果都不受影响。
- 组装因摘要不符（`digest_mismatch`）失败后，已收分块仍可补传，也可以直接重置整场会话；重置成功后再查会话状态，回到尚未开始的既有 409 口径（`chunks_not_started`），不留下半条记录。
- 路径标识为空或含 `/`、`\\`，请求携带任意查询参数，或声明了非空请求体，均返回 HTTP 400（错误码 `invalid_request`）。
- 标识合法但资源不存在时返回 HTTP 404（错误码 `resource_not_found`），不改动任何状态。
- `DELETE` 之外的方法返回 HTTP 405（错误码 `method_not_allowed`，`Allow` 头只给出 `DELETE`）。
- 并发上传与跨资源批量操作不在本接口范围内；失败请求不会留下半条会话。
