...
这是一个面向代码、AI 模型、数据集和构建产物的后端服务基线。当前版本提供可运行的 HTTP 服务、健康检查、进程内的资源登记、查询与按标识注销、资源之间的依赖关系登记、拓扑查询与影响分析接口、按资源标识提交原始字节的内容校验接口、内容寻址的分块存储、组装与成品读取接口、分块上传会话状态查询接口、资源生命周期状态（晋级、撤回与隔离）接口、按资源即时计算晋级阻塞明细的只读视图接口、按资源登记、查询、就地更新与删除安全告警（漏洞）的接口、把全部告警按公告编号汇总的全局只读视图接口、按公告编号下钻单条公告告警明细的全局只读视图接口、按公告编号下钻其命中组件修复建议的全局只读视图接口、按资源登记与查询 SBOM 文档与许可证声明的接口、按资源登记与查询构建来源证明（provenance）的接口、按资源登记与查询准入策略并执行只读准入评估的接口、按资源登记、查询与删除安全告警豁免（漏洞例外）并按豁免口径提供只读准入预览的接口、按资源即时计算风险评分的接口、按资源登记与查询通知记录的接口，以及在组件级漏洞关联视图之外、按资源即时计算只读的组件修复建议视图的接口，以及全局镜像层缓存的写入、读取、状态查询、单层删除与清空接口，以及全局镜像源及其签名策略的登记、查询与删除，和按摘要通过镜像源拉取镜像层、对一组层摘要按镜像源批量预取、按镜像源对登记上游发起探活并读取最近一次探活结果的接口，以及把本地资源指向其他仓库资源、登记时即时解析并建立依赖边的跨仓库引用接口，以及按资源登记、查询内容签名并对登记签名执行只读验签的接口。
...
按标识注销单个资源。请求不带请求体，也不接受任何查询参数。成功返回 HTTP 200，回显被删资源的完整登记内容，字段与键序与登记/查询响应完全一致，紧凑 UTF-8 JSON 并以单个换行结束：
...
```json
{"id":"…","name":"model-a","category":"model","digest":"a1b2c3d4…","source":"https://example.invalid/model-a"}
```

注销会连带清理该资源名下的全部记录，且立即在所有即时计算的视图中生效：

- 该资源名下的告警、告警豁免、SBOM 文档、许可证声明、构建来源证明、准入策略、内容签名与通知记录一并删除；全局公告汇总、公告明细、公告组件修复建议、组件视图、准入预览与风险评分立即按剩余告警重算，不再计入被删数据。
- 分块上传会话（含已收分块）与已组装的成品字节随之作废，此前未完成的上传进度不再可查。
- 生命周期状态一并移除；之后以相同类别、名称、摘要重新登记的资源回到默认的 `staged` 状态，不继承旧状态。
- 指向该资源的依赖边（它作为被依赖方）与由它发起的依赖边（它作为起点）都随之消失，依赖闭包、拓扑与影响分析不再出现这些边；依赖漏洞传导按剩余闭包与告警即时重算。
- 由该资源登记的跨仓库引用记录一并移除，对应 `(repository, remote_id)` 组合随即释放；引用解析时生成的远端本地资源保留不动。
- 此前签发的列表游标继续可用；翻页与筛选结果都不再包含被注销的资源。

注销只影响被选中的资源：其他资源的记录、全局镜像层缓存条目与镜像源登记一律不动。被删标识不再复用，后续新登记的资源一律获得新的服务生成标识。

#### 注销接口的错误

- 路径标识为空或含 `/`、`\\`，返回 HTTP 400（错误码 `invalid_request`），且不读取业务数据。
- 请求声明了非空或非法的请求体（非空或非合法十进制的 `Content-Length`），或携带任意查询参数，返回 HTTP 400（错误码 `invalid_request`），不改动任何既有状态；显式的 `Content-Length: 0` 与省略该头都视为空请求体。
- 标识格式合法但资源不存在，或资源此前已经注销（含对同一标识重复注销），返回 HTTP 404（错误码 `resource_not_found`）。
- 资源路径的查询方法保持不变；注销后同一路径的 `Allow` 头同时给出 `DELETE` 与 `GET`（`Allow: DELETE, GET`），其他未声明方法返回 HTTP 405（错误码 `method_not_allowed`）。

任何失败请求都不改动任何既有状态。资源与被删记录只存进程内存，停止或重启即清空，不生成任何持久化文件。
