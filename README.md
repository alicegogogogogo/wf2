...
这是一个面向代码、AI 模型、数据集和构建产物的后端服务基线。当前版本提供可运行的 HTTP 服务、健康检查、进程内的资源登记与查询、资源之间的依赖关系登记、拓扑查询与影响分析接口、按资源标识提交原始字节的内容校验接口、内容寻址的分块存储、组装与成品读取接口、分块上传会话状态查询接口、资源生命周期状态（晋级、撤回与隔离）接口、按资源登记与查询安全告警（漏洞）的接口、按资源登记与查询 SBOM 文档与许可证声明的接口，以及按资源登记与查询构建来源证明的接口。
...
可以为已登记的资源各登记一份构建来源证明，描述一次构建。来源证明只保存在当前进程内存中，服务停止或重启后随资源一起清空，不会写入任何文件，也不承诺跨进程同步。每个资源至多保留一份来源证明；本次不校验签名，也不改变生命周期晋级条件。

### 登记来源证明：`POST /resources/{id}/provenance`

请求体必须是一个完整的 JSON 对象，字段如下：

| 字段 | 类型 | 是否必填 | 说明 |
| --- | --- | --- | --- |
| `builder` | string | 是 | 构建者，非空，最多 256 个 Unicode 码点 |
| `build_id` | string | 是 | 构建编号，非空，最多 256 个 Unicode 码点 |
| `source_digest` | string | 是 | 64 位十六进制来源摘要（如 SHA-256），接受大小写混合，统一按小写存储与输出 |
| `materials` | array | 是 | 材料数组，可以为空数组；成员按提交顺序稳定保留 |

数组元素必须是 JSON 对象，且只包含 `name`、`digest` 两个字段：`name` 为非空字符串，最多 256 个 Unicode 码点；`digest` 必须是 64 位十六进制文本，接受大小写混合，响应中统一使用小写形式。材料成员不得缺字段、带未知字段或使用错误类型；同一数组中（规范化后）名称与摘要均相同的成员视为重复项。不允许额外未知字段。

首次登记成功返回 HTTP 201，响应体为紧凑 UTF-8 JSON，键序固定（`id`、`builder`、`build_id`、`source_digest`、`materials`）并以换行结束：

```bash
curl -s -X POST http://127.0.0.1:8000/resources/$ID/provenance \
  -H 'Content-Type: application/json' \
  -d '{"builder":"ci-bot","build_id":"42","source_digest":"A1B2C3D4E5F60000000000000000000000000000000000000000000000000000","materials":[{"name":"openssl","digest":"B2C3D4E5F6000000000000000000000000000000000000000000000000000000"}]}'
```

```json
{"id":"…","builder":"ci-bot","build_id":"42","source_digest":"a1b2c3d4e5f60000000000000000000000000000000000000000000000000000","materials":[{"name":"openssl","digest":"b2c3d4e5f6000000000000000000000000000000000000000000000000000000"}]}
```

幂等与冲突：

- 完全相同的规范化内容（构建者、构建编号、来源摘要及按顺序的全部材料均一致；大小写不同但规范化后相同的摘要视为一致）再次登记视为幂等，返回 HTTP 200，回显内容与首次登记相同，且不生成第二份记录。
- 材料按提交顺序保留，顺序不同即视为不同证明。
- 已有证明时提交任何不同内容（构建者、构建编号、来源摘要不同，或材料有任何增减、字段或顺序差异）返回 HTTP 409（错误码 `provenance_conflict`），原证明保持不变。

### 查询来源证明：`GET /resources/{id}/provenance`

返回该资源唯一一份来源证明，形状与登记响应相同，仍是 HTTP 200。资源存在但从未登记来源证明时返回 HTTP 404（错误码 `provenance_not_found`）。

### 来源证明接口的错误

下列情况都返回 HTTP 400（错误码 `invalid_request`），且不写入或修改任何来源证明或其他状态：

- 请求体缺失、不是合法 UTF-8、无法解码为 JSON，或顶层不是 JSON 对象；
- 缺少 `builder`、`build_id`、`source_digest`、`materials` 任一字段，或出现未知字段；
- `builder`、`build_id` 不是字符串、为空或超过 256 个 Unicode 码点；
- `source_digest` 不是 64 位十六进制文本；
- `materials` 不是数组，或其成员不是对象、缺少 `name`/`digest`、带未知字段、字段类型错误、`name` 为空或超过 256 码点、`digest` 不是 64 位十六进制文本，或成员（规范化后）重复；
- 路径标识为空或含 `/`、`\\`；GET、POST 请求携带任意查询参数。

路径标识格式合法但资源不存在时，POST 与 GET 都返回 HTTP 404（错误码 `resource_not_found`）；资源存在但来源证明缺失时 GET 返回 404（`provenance_not_found`）。对该路径使用 `GET`、`POST` 之外的方法返回 HTTP 405（错误码 `method_not_allowed`，`Allow: GET, POST`）。

任何失败都不会修改资源、依赖、分块、成品、生命周期状态、游标、安全告警、SBOM 文档、许可证或来源证明；来源证明仅存于当前进程内存，停止或重启即清空，不生成任何文件，本次也不校验签名或改变晋级条件。
...
- 当前公开业务接口为健康检查、上述资源登记/查询接口、资源依赖关系登记、依赖拓扑查询与影响分析接口、资源内容校验接口、分块上传、组装、分块上传会话状态查询与成品读取接口，资源生命周期状态读取与提交接口，安全告警（漏洞）的登记与查询接口，SBOM 文档与许可证声明的登记与查询接口，以及构建来源证明的登记与查询接口；资源、依赖、分块会话、成品内容、生命周期状态、安全告警、SBOM 文档、许可证声明与构建来源证明均仅存于进程内存，不承诺跨进程或重启后的保存。
