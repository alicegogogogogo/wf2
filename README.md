...
可以在同一读取入口加 HTTP 标准的 `Range` 请求头，只取成品的一段字节。范围条件只支持单段字节区间，单位必须是 `bytes`：

| 写法 | 含义 |
| --- | --- |
| `bytes=START-END` | 从 `START` 到 `END`（含两端）的字节 |
| `bytes=START-` | 从 `START` 到成品末尾（开区间末端） |
| `bytes=-SUFFIX` | 成品最后 `SUFFIX` 个字节（`SUFFIX` 为正整数，`-0` 视为语法非法） |

`START`、`END` 都是十进制非负整数。区间两端都可写成开区间；末端超出成品长度时按实际剩余字节收口。命中时返回 HTTP 206，`Content-Type` 与整份读取一致（`application/octet-stream`），并给出标准 `Content-Range`（`bytes START-END/总长度`），响应体只含命中的原始字节，`Content-Length` 等于这段字节的准确数目：
...
- 范围单位不是 `bytes`、语法非法，或写成多段（含逗号的多个区间），返回 HTTP 400（错误码 `invalid_request`）；后缀长度写成零或负数（如 `-0`、`--1`）同样按语法非法处理。
- 起始位置不小于成品总长度时，返回 HTTP 416（错误码 `range_not_satisfiable`），并带 `Content-Range: bytes */总长度` 给出成品总长度。空成品的任何范围条件都不可满足（同样返回 416，`Content-Range` 为 `bytes */0`）。
- 范围条件不改变既有判定顺序：资源不存在仍是 404、尚未组装完成仍是 409、标识非法或带查询参数仍是 400、非 `GET` 方法仍是 405（`Allow: GET`）。

不带 `Range` 头时整份返回，状态码与响应字段与原先一字不变。任何读取（无论是否带范围）都不写入状态、不留记录，也不改变缓存条目与计数。
