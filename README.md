...
这是一个面向代码、AI 模型、数据集和构建产物的后端服务基线。当前版本提供可运行的 HTTP 服务、健康检查、进程内的资源登记与查询、资源之间的依赖关系登记、拓扑查询与影响分析接口、按资源标识提交原始字节的内容校验接口、内容寻址的分块存储、组装与成品读取接口、分块上传会话状态查询接口、资源生命周期状态（晋级、撤回与隔离）接口、按资源登记与查询安全告警（漏洞）的接口、按资源登记与查询 SBOM 文档与许可证声明的接口、按资源登记与查询构建来源证明（provenance）的接口、按资源登记与查询准入策略并执行只读准入评估的接口、按资源即时计算风险评分的接口、按资源即时把已登记 SBOM 组件与安全告警按组件名称关联的组件级漏洞视图接口、按资源登记与查询通知记录的接口，以及全局镜像层缓存的写入、读取、状态查询、单层删除与清空接口，以及全局镜像源的登记、查询与按摘要通过镜像源拉取镜像层的接口，以及把本地资源指向其他仓库资源、登记时即时解析并建立依赖边的跨仓库引用接口，以及按资源登记、查询内容签名并对登记签名执行只读验签的接口。
...
在按资源分开的 SBOM 与安全告警查询之外，可以只读查询一份把两者按组件名称关联起来的视图。视图在请求时即时计算，只读取该资源已登记的 SBOM 与安全告警，**不写入任何状态**，也不留下任何记录；停止或重启后随输入一起清空，不写入任何文件。

### 查询组件风险：`GET /resources/{id}/component-risks`

服务把已登记 SBOM 的每个组件与该资源的安全告警按组件名称关联。关联时告警里的组件名与 SBOM 组件名做**完整字符串比较，区分大小写**：不做模糊匹配，也不做空白修剪，名称逐字相同才算命中；版本与摘要（digest）不参与比较。

成功返回 HTTP 200，响应体为紧凑 UTF-8 JSON，顶层字段是 `components`，正文以换行结束：
...
```json
{"components":[{"name":"openssl","version":"3.0.8","digest":"a1b2…","advisories":[{"id":"…","severity":"high"}]},{"name":"zlib","version":"1.3","digest":"b2c3…","advisories":[]}]}
```

| 字段 | 说明 |
| --- | --- |
| `components` | SBOM 中的全部组件，**按 SBOM 提交顺序排列**；未命中任何告警的组件照常输出 |
| `name` / `version` / `digest` | 组件的名称、版本与摘要，字段名与顺序依次为 `name`、`version`、`digest`（摘要沿用 SBOM 的小写形式） |
| `advisories` | 命中该组件名称的告警数组；未命中时为空数组 `[]` |
| `advisories[].id` | 告警登记时由服务生成的告警标识 |
| `advisories[].severity` | 告警的严重级别，按告警登记时的小写形式输出 |

同一组件命中的告警**按该资源告警的登记顺序排列，不去重也不合并**。该资源没有任何组件、组件没有任何命中，或资源没有任何告警时都返回空集合（`{"components":[]}` 或各组件 `advisories` 为空数组），同样是成功响应。

### 组件风险视图的错误

- 路径标识为空或含 `/`、`\\`，返回 HTTP 400（错误码 `invalid_request`），且不读取业务数据。
- 请求携带任意查询参数，返回 HTTP 400（错误码 `invalid_request`）；本接口不提供筛选或分页。
- 标识格式合法但资源不存在，返回 HTTP 404（错误码 `resource_not_found`），状态保持不变。
- 资源存在但从未登记 SBOM，返回 HTTP 404（错误码 `sbom_not_found`），不生成任何记录。
- `GET` 之外的方法返回 HTTP 405（错误码 `method_not_allowed`，`Allow: GET`）。

该查询即时计算且只读，不改变告警、SBOM 与其他既有记录；既有告警、SBOM、准入、风险与签名接口的字段、错误码与响应形状保持不变。
...
- 当前公开业务接口为健康检查、上述资源登记/查询接口、资源依赖关系登记、依赖拓扑查询与影响分析接口、资源内容校验接口、分块上传、组装、分块上传会话状态查询与成品读取接口，资源生命周期状态读取与提交接口，安全告警（漏洞）的登记与查询接口，SBOM 文档与许可证声明的登记与查询接口，构建来源证明（provenance）的登记与查询接口，准入策略的登记与查询接口和只读准入评估接口，按资源即时计算的风险评分接口，按资源即时计算、把已登记 SBOM 组件与安全告警按组件名称逐字关联的组件级漏洞视图接口，以及通知记录的登记与查询接口，以及全局镜像层缓存的写入、读取、状态查询、单层删除与清空接口，以及全局镜像源的登记、查询和通过镜像源按摘要拉取镜像层的接口，以及把本地资源指向其他仓库资源、登记时即时解析远端元数据并建立依赖边的跨仓库引用登记与查询接口，以及内容签名的登记、查询与只读验签接口；资源、依赖、分块会话、成品内容、生命周期状态、安全告警、SBOM 文档、许可证声明、构建来源证明、准入策略、通知记录、镜像源登记、跨仓库引用、内容签名记录与缓存条目及计数均仅存于进程内存（跨仓库引用解析失败不留引用、不建边、不生成本地资源；缓存的删除与清空只移除条目并回落已用字节，不改动资源、依赖、成品、镜像源、跨仓库引用等既有状态，也不重置命中与未命中计数），准入评估结果、风险评分、组件风险视图与签名验签均即时计算、不落记录、不承诺跨进程或重启后的保存。
