...
这是一个面向代码、AI 模型、数据集和构建产物的后端服务基线。当前版本提供可运行的 HTTP 服务、健康检查、进程内的资源登记与查询、资源之间的依赖关系登记、拓扑查询与影响分析接口、按资源标识提交原始字节的内容校验接口、内容寻址的分块存储、组装与成品读取接口、分块上传会话状态查询接口、资源生命周期状态（晋级、撤回与隔离）接口、按资源登记与查询安全告警（漏洞）的接口，以及按资源登记与查询 SBOM（软件物料清单）与许可证声明的接口。
...
可以为每个已登记资源保存一份 SBOM 文档。文档只保存在当前进程内存中，服务停止或重启后随资源一起清空，不会写入任何文件；每个资源至多保留一份，不提供更新、删除或批量能力。
...
| `components` | array | 是 | 组件数组，按提交顺序保留，可以为空数组 `[]` |

每个数组元素必须是 JSON 对象，且只允许以下三个字段，均为非空字符串：

| 字段 | 类型 | 说明 |
| --- | --- | --- |
| `name` | string | 组件名称，非空 |
| `version` | string | 组件版本，非空 |
| `digest` | string | 64 位十六进制组件摘要（接受大小写混合），响应中统一为小写 |

不允许额外未知字段（顶层或组件元素内均如此）。首次登记成功返回 HTTP 201，响应体为紧凑 UTF-8 JSON，键序固定（`id`、`format`、`components`；组件内为 `name`、`version`、`digest`）并以换行结束：

```bash
curl -s -X POST http://127.0.0.1:8000/resources/$ID/sbom \
  -H 'Content-Type: application/json' \
  -d '{"format":"spdx","components":[{"name":"openssl","version":"3.0.9","digest":"AAAA00000000000000000000000000000000000000000000000000000000FFFF"}]}'
```

```json
{"id":"…","format":"spdx","components":[{"name":"openssl","version":"3.0.9","digest":"aaaa00000000000000000000000000000000000000000000000000000000ffff"}]}
```

幂等与冲突：

- 同一份文档（格式与全部组件三元组逐字相同；摘要按小写归一化后比较）再次提交视为幂等重试，返回 HTTP 200，回显相同内容，且不生成第二份记录。
- 提交的组件数组内出现重复的 `name`/`version`/`digest` 三元组，返回 HTTP 409（错误码 `duplicate_component`），不写入文档。
- 已有文档时提交任何不同内容（格式不同、组件数量或任一三元组不同、顺序不同），返回 HTTP 409（错误码 `sbom_conflict`），原文档保持不变。

### 查询 SBOM：`GET /resources/{id}/sbom`

成功返回 HTTP 200，响应内容与登记成功时完全一致（`id`、`format`、`components`，组件保持提交顺序、摘要为小写）。该资源尚未登记 SBOM 时返回 HTTP 404（错误码 `sbom_not_found`）。

## 许可证声明

可以为每个已登记资源保存一份许可证声明。声明只保存在当前进程内存中，服务停止或重启后随资源一起清空，不会写入任何文件；每个资源至多保留一份，不提供更新、删除或批量能力。

### 登记许可证：`POST /resources/{id}/license`

请求体必须是一个完整的 JSON 对象，且只允许以下字段：

| 字段 | 类型 | 是否必填 | 说明 |
| --- | --- | --- | --- |
| `spdx_id` | string | 是 | 非空 SPDX 许可证标识，有值时原样回显（不做大小写归一化或合法性查表） |
| `source` | string | 否 | 非空来源描述；省略时为 `null`，提供时原样回显。显式传 `null` 属于类型错误 |

不允许额外未知字段。首次登记成功返回 HTTP 201，响应体为紧凑 UTF-8 JSON，键序固定（`id`、`spdx_id`、`source`）并以换行结束：
...
```json
{"id":"…","spdx_id":"Apache-2.0","source":"https://example.invalid/LICENSE"}
```

幂等与冲突：

- 相同内容（`spdx_id` 与 `source` 均相同，包括省略与提供 `source` 的区别）再次提交返回 HTTP 200，不生成第二份记录。
- 已有声明时提交任何不同内容（不同 `spdx_id`、补填或去掉 `source`、`source` 不同），返回 HTTP 409（错误码 `license_conflict`），原声明保持不变。

### 查询许可证：`GET /resources/{id}/license`

成功返回 HTTP 200，响应内容与登记成功时完全一致。该资源尚未登记许可证时返回 HTTP 404（错误码 `license_not_found`）。

### SBOM 与许可证接口的错误

下列规则对 `/resources/{id}/sbom` 与 `/resources/{id}/license` 同样适用：

- 请求体缺失、不是合法 UTF-8、无法解码为 JSON，或顶层不是 JSON 对象，返回 HTTP 400（错误码 `invalid_request`）。
- 缺少必填字段、出现未知字段、字段类型错误、字段为空（含显式 `null`），或 SBOM 的 `format` 非法、`components` 不是数组、组件元素不是对象/缺字段/含未知字段/摘要不是 64 位十六进制，均返回 HTTP 400（错误码 `invalid_request`）。
- 路径标识为空或含 `/`、`\\`，或携带任意查询参数，返回 HTTP 400（错误码 `invalid_request`）。
- 路径标识格式合法但资源不存在，POST 与 GET 都返回 HTTP 404（错误码 `resource_not_found`）；资源存在但尚未登记对应文档时，GET 返回 404（`sbom_not_found` 或 `license_not_found`）。
- 对这两个路径使用 `GET`、`POST` 之外的方法返回 HTTP 405（错误码 `method_not_allowed`，`Allow: GET, POST`）。
- 任何失败都不会新增或修改资源、依赖、分块、成品、生命周期状态、漏洞、SBOM、许可证或列表游标。
...
- 当前公开业务接口为健康检查、上述资源登记/查询接口、资源依赖关系登记、依赖拓扑查询与影响分析接口、资源内容校验接口、分块上传、组装、分块上传会话状态查询与成品读取接口，资源生命周期状态读取与提交接口，安全告警（漏洞）的登记与查询接口，以及 SBOM 文档与许可证声明的登记和查询接口；资源、依赖、分块会话、成品内容、生命周期状态、安全告警、SBOM 文档与许可证声明均仅存于进程内存，不承诺跨进程或重启后的保存。
