...
这是一个面向代码、AI 模型、数据集和构建产物的后端服务基线。当前版本提供可运行的 HTTP 服务、健康检查、进程内的资源登记与查询、资源之间的依赖关系登记、拓扑查询与影响分析接口、按资源标识提交原始字节的内容校验接口、内容寻址的分块存储、组装与成品读取接口、分块上传会话状态查询接口、资源生命周期状态（晋级、撤回与隔离）接口、按资源即时计算晋级阻塞明细的只读视图接口、按资源登记与查询安全告警（漏洞）的接口、把全部告警按公告编号汇总、即时计算且只读的全局告警汇总视图接口、按资源登记与查询 SBOM 文档与许可证声明的接口、按资源登记与查询构建来源证明（provenance）的接口、按资源登记与查询准入策略并执行只读准入评估的接口、按资源登记、查询与删除安全告警豁免（漏洞例外）并按豁免口径提供只读准入预览的接口、按资源即时计算风险评分的接口、按资源登记与查询通知记录的接口，以及在组件级漏洞关联视图之外、按资源即时计算只读的组件修复建议视图的接口，以及全局镜像层缓存的写入、读取、状态查询、单层删除与清空接口，以及全局镜像源及其签名策略的登记、查询与删除，和按摘要通过镜像源拉取镜像层、按镜像源对登记上游发起探活并读取最近一次探活结果的接口，以及把本地资源指向其他仓库资源、登记时即时解析并建立依赖边的跨仓库引用接口，以及按资源登记、查询内容签名并对登记签名执行只读验签的接口。
...
在按资源分别查看告警之外，可以把当前进程中全部资源的安全告警按公告编号即时汇总成一份全局视图。该视图**即时计算且只读**，不落任何记录，也不改变告警、资源、准入评估、豁免与风险评分的结果；数据仍只保存在当前进程内存中，服务停止或重启后随告警一起清空，不写入任何文件。

### 查询全局告警汇总：`GET /advisories`

只接受 `GET` 查询，请求不接受请求体。成功返回 HTTP 200，响应体是紧凑 UTF-8 JSON 数组并以单个换行结束；没有任何告警时返回空数组 `[]`，同样是成功响应。每条记录固定四个字段，顺序为 `advisory`、`affected_resources`、`advisory_count`、`max_severity`：
...
| --- | --- |
| `advisory` | 公告编号，原样输出 |
| `affected_resources` | 该公告下出现告警的资源标识数组，按资源登记顺序排列，每个资源只出现一次，不受告警提交先后影响 |
| `advisory_count` | 该公告下的告警记录总数，不去重也不合并；同一资源贡献的多条记录照常累加 |
| `max_severity` | 该公告全部告警里的最高严重度，四档从高到低固定为 `critical`、`high`、`medium`、`low`，比较忽略大小写，按小写输出 |

- 公告按**首次出现的顺序**展开，即按该公告最早一条对应告警的提交先后排列，与资源登记先后无关。
- `affected_resources` 只决定资源在数组内的排列；某资源是否出现取决于它是否有命中的告警记录。
- 筛选给出后，只统计命中级别的告警再分组：组内资源去重与登记顺序排列、计数不去重不合并、最高严重度取命中告警的口径均不变；筛选后没有任何命中时返回空数组 `[]`，是正常响应而不是错误。

#### 严重度筛选

可使用 `severity` 查询参数，取值为 `critical`、`high`、`medium`、`low`，比较忽略大小写：
...
- 请求方法不是 `GET` 时返回 HTTP 405（错误码 `method_not_allowed`，`Allow: GET`）。
- 请求声明非空请求体（声明非空或非法的 `Content-Length` 同样拒绝）时返回 HTTP 400（错误码 `invalid_request`），不读取业务数据。
- 查询参数只允许严重度筛选：出现未知或重复参数（含重复的 `severity`）返回 HTTP 400（错误码 `invalid_request`）。
- `severity` 为空值或不在四个合法级别之内时返回 HTTP 400（错误码 `invalid_request`）。
- 错误响应体沿用既有 JSON 错误形状（`{"error":"<稳定错误码>","message":"<说明>"}`）并以单个换行结束。

该视图不改变告警登记与查询的既有语义，也不改准入评估、豁免预览、风险评分和组件视图的口径；签名请求头取值非法返回 400 的口径只适用于单摘要拉取这一条链路，与本视图无关。
