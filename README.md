...
这是一个面向代码、AI 模型、数据集和构建产物的后端服务基线。当前版本提供可运行的 HTTP 服务、健康检查、进程内的资源登记与查询、资源之间的依赖关系登记、拓扑查询与影响分析接口、按资源标识提交原始字节的内容校验接口、内容寻址的分块存储、组装与成品读取接口、分块上传会话状态查询接口、资源生命周期状态（晋级、撤回与隔离）接口、按资源即时计算晋级阻塞明细的只读视图接口、按资源登记与查询安全告警（漏洞）的接口、按资源登记与查询 SBOM 文档与许可证声明的接口、按资源登记与查询构建来源证明（provenance）的接口、按资源登记与查询准入策略并执行只读准入评估的接口、按资源登记、查询与删除安全告警豁免（漏洞例外）并按豁免口径提供只读准入预览的接口、按资源即时计算风险评分的接口、按资源登记与查询通知记录的接口，以及在组件级漏洞关联视图之外、按资源即时计算只读的组件修复建议视图的接口，以及全局镜像层缓存的写入、读取、状态查询、单层删除与清空接口，以及全局镜像源及其签名策略的登记、查询与删除，和按摘要通过镜像源拉取镜像层、对镜像源上游发起探活并读取最近一次探活结果的接口，以及把本地资源指向其他仓库资源、登记时即时解析并建立依赖边的跨仓库引用接口，以及按资源登记、查询内容签名并对登记签名执行只读验签的接口。
...
路径资源标识格式合法但资源不存在时，各入口都返回 HTTP 404（错误码 `resource_not_found`）；资源存在但缺少策略时，准入预览返回 HTTP 404（错误码 `policy_not_found`）。方法不符返回 HTTP 405（错误码 `method_not_allowed`，附带 `Allow` 头）：豁免集合为 `Allow: GET, POST`，单条豁免为 `Allow: DELETE`，准入预览为 `Allow: GET`。
...
可以登记若干镜像源，通过某个镜像源按摘要拉取镜像层，并对镜像源的登记上游地址发起探活。镜像源记录只保存在当前进程内存中，重启即清空，不写入任何文件；拉取复用全局 `/cache` 层缓存：命中既有缓存时直接返回原始字节，未命中时向登记的上游地址取回、校验摘要后写入缓存再返回。探活结果同样只存进程内存，每个镜像源只保留最近一次结果；探活不改变缓存条目与计数。镜像源之间相互独立，名称全局唯一。
...
删除指定镜像源。成功返回 HTTP 200，响应体回显被删记录的 `id`、`name`、`upstream` 三个字段，键序与登记响应一致，仍是紧凑 UTF-8 JSON 并以单个换行结束。删除会连带清掉该源名下的签名策略与最近一次探活结果，但回显仍只含镜像源三项；删除不影响缓存条目、命中与未命中计数，也不影响跨仓库引用、资源依赖与成品内容。同名镜像源删除后允许重新登记。标识合法但镜像源不存在或已删除时返回 HTTP 404（错误码 `mirror_not_found`），对同一镜像源重复删除同样返回 404，不产生任何新的副作用。
...
单项失败不影响其余项继续处理，全部失败时同样返回 HTTP 200 并逐项报告。配额不足、摘要不符或签名失败只影响该项，不回滚其他项已写入的缓存。镜像源已登记签名策略时，每个摘要都用同一组签名请求头（`X-Key-Id`、`X-Signature`、`X-Signed-Digest`）逐项校验，覆盖检查（`cover_digest` 为 `true` 时）对照各自被拉摘要；签名校验按既有判定顺序逐项执行，缺头、密钥、覆盖、签名命中即止，只报一个错误码。多项条件同时命中时只报排在最前的错误码：缺头报 `signature_missing`；头都在时，未受信密钥即使伴随其他头取值非法（如签名为空、被签摘要不是 64 位十六进制）也只报 `key_not_trusted`，不再出现 `invalid_request`、`digest_uncovered` 或 `signature_invalid`；密钥受信时覆盖先于签名值。头都在但取值本身非法、且密钥受信的情况才报 `invalid_request`。未登记策略时签名头被忽略。缓存条目的命中与未命中计数仍按既有拉取规则变化（预取的命中判定使用不计计数的读取，与单摘要拉取一致），本接口不改计数口径。

预取结果与写入的缓存条目只存进程内存，重启清空，不生成文件。

### 镜像源探活：`POST /mirrors/{id}/probe` 与 `GET /mirrors/{id}/probe`

在镜像源标识后挂探活入口，对登记的上游地址做一次真实连接尝试。探活只存进程内存、重启清空、不写文件，也不改变缓存条目与命中、未命中计数；每个镜像源只保留最近一次探活结果，后一次覆盖前一次。

`POST` 触发一次真实探活：按登记的上游地址（不做路径拼接）发起一次 GET 连接尝试，记录结果并覆盖该镜像源上一次的探活结果。`POST` 不接受请求体与查询参数（显式 `Content-Length: 0` 的空请求体是合法的）。`GET` 只读取最近一次探活结果，不发起连接、不写任何状态。两者成功时都返回 HTTP 200，响应体是紧凑 UTF-8 JSON 一行并以单个换行结束，键序固定为 `id`、`reachable`、`status_code`、`latency_ms`：

- 上游对连接尝试给出任何完整 HTTP 响应（含 4xx、5xx 等错误状态）都视为可达：`reachable` 为 `true`，`status_code` 是上游响应码，`latency_ms` 是本次尝试的非负整数耗时毫秒。
- 上游连接失败、超时等无法取得响应时视为不可达：`reachable` 为 `false`，`status_code` 为 `null`，`latency_ms` 仍为非负整数耗时毫秒，响应在四项之后再带一个 `error` 字段，值是稳定错误码 `probe_failed`。
- 可达与不可达都是 HTTP 200 的正常业务结果，不视为服务错误。
...
```json
{"id":"…","reachable":true,"status_code":200,"latency_ms":12}
```

不可达时：

```json
{"id":"…","reachable":false,"status_code":null,"latency_ms":7,"error":"probe_failed"}
```

随后 `GET /mirrors/{id}/probe` 返回与上述 `POST` 完全相同的成功形状，但不重新探活。

探活的错误与边界：

- 路径标识为空或含 `/`、`\`，或请求携带任意查询参数，`GET` 与 `POST` 都返回 HTTP 400（错误码 `invalid_request`）；`POST` 声明非空或非法的请求体同样返回 400，且不发起连接、不留下探活结果。
- 标识合法但镜像源不存在时，两个入口都返回 HTTP 404（错误码 `mirror_not_found`）。
- 镜像源存在但从未探活时，`GET` 返回 HTTP 404（错误码 `probe_not_found`），不生成任何结果。
- 探活入口只允许 `GET` 与 `POST`；其他方法返回 HTTP 405（错误码 `method_not_allowed`，`Allow: GET, POST`）。
- 删除镜像源会连带清掉其探活结果；探活不影响镜像源、签名策略、缓存、计数与其他既有入口的行为。
...
2. `X-Key-Id` 不在策略的 `keys` 中，返回 HTTP 403（错误码 `key_not_trusted`）。只要三个头都在，未受信密钥即使同时伴随其他头取值非法（如签名为空、被签摘要不是 64 位十六进制），也只报 `key_not_trusted`，不再报 `invalid_request` 或更靠后的错误码。
...
三个头都在、密钥受信，但 `X-Key-Id`、`X-Signature` 为空，或 `X-Signed-Digest` 不是 64 位十六进制，才按头取值非法返回 HTTP 400（错误码 `invalid_request`）。
...
- 探活 `POST` 声明非空或非法的请求体（探活 `GET` 不读取请求体）；
- 两个删除入口携带请求体；
- 任意镜像源接口携带查询参数（批量预取与探活携带任意查询参数时同样返回 400，且不写入任何缓存条目或探活结果）。

路径标识格式合法但镜像源不存在时，单条查询、删除、拉取、批量预取、探活与签名策略接口都返回 HTTP 404（错误码 `mirror_not_found`），且不改变状态；镜像源存在但从未探活时，探活 `GET` 返回 HTTP 404（错误码 `probe_not_found`）。方法不符返回 HTTP 405（错误码 `method_not_allowed`，响应带 `Allow` 头）：`/mirrors` 为 `Allow: GET, POST`，`/mirrors/{id}` 为 `Allow: DELETE, GET`，`/mirrors/{id}/pull/{digest}` 为 `Allow: POST`，`/mirrors/{id}/prefetch` 为 `Allow: POST`，`/mirrors/{id}/probe` 为 `Allow: GET, POST`，`/mirrors/{id}/signature-policy` 为 `Allow: DELETE, GET, POST`。

镜像源数据、签名策略、最近一次探活结果与由拉取写入的缓存条目均仅存于当前进程内存，停止或重启即清空（删除镜像源同时清掉其签名策略与探活结果），不生成任何文件；缓存条目的删除与清空通过上文 `/cache` 的两个删除入口完成，镜像源的并发处理不在当前范围内。
...
- 当前公开业务接口为健康检查、上述资源登记/查询接口、资源依赖关系登记、依赖拓扑查询与影响分析接口、资源内容校验接口、分块上传、组装、分块上传会话状态查询与成品读取接口，资源生命周期状态读取与提交接口，以及按既有晋级检查顺序即时给出 `content_not_complete`、`dependency_blocked` 原因并逐条列出被隔离或撤回的可达依赖、即时计算且只读不落记录的晋级阻塞明细视图（`release-blockers`）接口，安全告警（漏洞）的登记与查询接口，SBOM 文档与许可证声明的登记与查询接口，构建来源证明（provenance）的登记与查询接口，准入策略的登记与查询接口和只读准入评估接口，按资源登记、查询与删除安全告警豁免（漏洞例外）的接口，以及判定与准入评估一致、但超限严重度只统计未豁免告警并返回 `exempted_count` 的只读准入预览接口（`admission-preview`），按资源即时计算的风险评分接口，把已登记 SBOM 的组件与该资源安全告警按组件名称逐字、区分大小写关联、即时计算且只读的组件级漏洞关联视图（`component-risks`）接口，以及同样按名称逐字关联、即时计算且只读、给出当前版本与点分十进制最大非空修复版本建议的组件修复建议视图（`component-fixes`）接口，以及通知记录的登记与查询接口，以及全局镜像层缓存的写入、读取、状态查询、单层删除与清空接口，以及全局镜像源及其签名策略的登记、查询与删除，和通过镜像源按摘要拉取镜像层、对镜像源上游发起探活并读取最近一次探活结果的接口，以及把本地资源指向其他仓库资源、登记时即时解析远端元数据并建立依赖边的跨仓库引用登记与查询接口，以及内容签名的登记、查询与只读验签接口，以及沿依赖闭包把每个资源的告警总数与最高严重度即时汇总、只读不落记录的依赖漏洞传导视图（`dependency-vulnerability-impact`）接口；资源、依赖、分块会话、成品内容、生命周期状态、安全告警、安全告警豁免、SBOM 文档、许可证声明、构建来源证明、准入策略、通知记录、镜像源登记、镜像源最近一次探活结果、跨仓库引用、内容签名记录与缓存条目及计数均仅存于进程内存（跨仓库引用解析失败不留引用、不建边、不生成本地资源；缓存的删除与清空只移除条目并回落已用字节，不改动资源、依赖、成品、镜像源、探活结果、跨仓库引用等既有状态，也不重置命中与未命中计数；删除镜像源连带清掉其签名策略与探活结果，但不动缓存），准入评估结果、准入预览结果、风险评分、晋级阻塞明细与签名验签均即时计算、不落记录、不承诺跨进程或重启后的保存。
